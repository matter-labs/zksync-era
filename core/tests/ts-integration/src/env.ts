...
export async function waitForServer(l2NodeUrl: string) {
  ...
  for (let i = 0; i < maxAttempts; ++i) {
    try {
      ...
      const bridgeAddress = (await l2Provider.getDefaultBridgeAddresses())
        .sharedL2;
      ...
      reporter.message(
        `Attempt #${i + 1} to check the server readiness failed`,
      );
...
function getMainWalletPk(pathToHome: string): string {
  if (process.env.MASTER_WALLET_PK) {
    ...
    let pk = ethers.Wallet.fromPhrase(
...
async function loadTestEnvironmentFromFile(
  ...
  let configsFolderSuffix =
    nodeMode == NodeMode.External ? "external_node" : undefined;
  let generalConfig = loadConfig({
    pathToHome,
    chain,
    config: "general.yaml",
    configsFolderSuffix,
  });
  let secretsConfig = loadConfig({
    pathToHome,
    chain,
    config: "secrets.yaml",
    configsFolderSuffix,
  ...
  const l1BatchCommitDataGeneratorMode =
    genesisConfig.l1_batch_commit_data_generator_mode as DataAvailabityMode;
  const minimalL2GasPrice = BigInt(
    generalConfig.state_keeper.minimal_l2_gas_price,
  );

  const validationComputationalGasLimit = parseInt(
    generalConfig.state_keeper.validation_computational_gas_limit,
  );
  // TODO set it properly
  const priorityTxMaxGasLimit = 72000000n;
  const maxLogsLimit = parseInt(
    generalConfig.api.web3_json_rpc.req_entities_limit,
  );
  ...
  const timestampAsserterMinTimeTillEndSec = parseInt(
    generalConfig.timestamp_asserter.min_time_till_end_sec,
  );
...
export async function loadTestEnvironmentFromEnv(): Promise<TestEnvironment> {
  ...
  let token = tokens.find(
    (token: { symbol: string }) => token.symbol == "WBTC",
  )!;
  if (!token) {
    token = tokens[0];
  }
  ...
  const l1BatchCommitDataGeneratorMode = (process.env
    .CHAIN_STATE_KEEPER_L1_BATCH_COMMIT_DATA_GENERATOR_MODE ||
  ...
  if (process.env.CHAIN_STATE_KEEPER_MINIMAL_L2_GAS_PRICE !== undefined) {
    minimalL2GasPrice = BigInt(
      process.env.CHAIN_STATE_KEEPER_MINIMAL_L2_GAS_PRICE!,
    );
  } else {
  ...
  const priorityTxMaxGasLimit = BigInt(
    process.env.CONTRACTS_PRIORITY_TX_MAX_GAS_LIMIT!,
  );
  const maxLogsLimit = parseInt(
    process.env.EN_REQ_ENTITIES_LIMIT ??
      process.env.API_WEB3_JSON_RPC_REQ_ENTITIES_LIMIT!,
  );
  ...
  const timestampAsserterAddress =
    process.env.CONTRACTS_L2_TIMESTAMP_ASSERTER_ADDR.toString();

  const timestampAsserterMinTimeTillEndSec = parseInt(
    process.env.TIMESTAMP_ASSERTER_MIN_TIME_TILL_END_SEC!,
  );
...
function getTokensNew(pathToHome: string): Tokens {
  ...
  for (const key in parsedObject.tokens) {
    parsedObject.tokens[key].decimals = BigInt(
      parsedObject.tokens[key].decimals,
    );
  }
  return parsedObject;
}
