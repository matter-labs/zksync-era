...
#[derive(Debug)]
pub struct FriProverJobArchivation {
    pool: ConnectionPool<Prover>,
    reporting_interval_ms: u64,
    archiving_interval_ms: u64,
}

impl FriProverJobArchivation {
    pub fn new(
        pool: ConnectionPool<Prover>,
        reporting_interval_ms: u64,
        archiving_interval_ms: u64,
...
impl PeriodicJob for FriProverJobArchivation {
    const SERVICE_NAME: &'static str = "FriProverJobArchivation";
    ...
    async fn run_routine_task(&mut self) -> anyhow::Result<()> {
        let archived_jobs = self
            ...
            .unwrap()
            .fri_prover_jobs_dal()
            .archive_old_jobs(self.archiving_interval_ms)
            .await;
