...
const BUILTIN_CREATE2_FACTORY_ADDRESS =
...
describe("System behavior checks", () => {
  ...
  test("Should check that system contracts and SDK create same CREATE/CREATE2 addresses", async () => {
    ...
    const create2AddressBySDK = zksync.utils.create2Address(
      sender,
      hash,
      salt,
      input,
    );
    const create2AddressByDeployer =
      await deployerContract.getNewAddressCreate2(sender, hash, salt, input);
  ...
  test("Should check bootloader utils: EIP2930 tx hash", async () => {
    ...
    const expectedEIP2930SignedHash = ethers.keccak256(
      parsedEIP2930tx.unsignedSerialized,
    );

    const proposedEIP2930Hashes =
      await bootloaderUtils.getTransactionHashes(EIP2930TxData);
    expect(proposedEIP2930Hashes.txHash).toEqual(expectedEIP2930TxHash);
    expect(proposedEIP2930Hashes.signedTxHash).toEqual(
      expectedEIP2930SignedHash,
    );
  });
  ...
  test("Should check bootloader utils: EIP1559 tx hash", async () => {
    ...
    const expectedEIP1559SignedHash = ethers.keccak256(
      parsedEIP1559tx.unsignedSerialized,
    );

    const proposedEIP1559Hashes =
      await bootloaderUtils.getTransactionHashes(EIP1559TxData);
    expect(proposedEIP1559Hashes.txHash).toEqual(expectedEIP1559TxHash);
    expect(proposedEIP1559Hashes.signedTxHash).toEqual(
      expectedEIP1559SignedHash,
    );
  });
  ...
  test("Should check bootloader utils: EIP712 tx hash", async () => {
    ...
    const expectedEIP712SignedHash =
      zksync.EIP712Signer.getSignedDigest(eip712Tx);

    const proposedEIP712Hashes =
      await bootloaderUtils.getTransactionHashes(eip712TxData);
  ...
  test("Should execute withdrawals with different parameters in one block", async () => {
    ...
    // Fund Bob's account.
    await alice
      .transfer({ amount, to: bob.address, token: l2Token })
      .then((tx) => tx.wait());
    ...
    const aliceChange = await shouldChangeTokenBalances(
      l1Token,
      [{ wallet: alice, change: amount }],
      { l1: true },
    );
    const bobChange = await shouldChangeTokenBalances(
      l1Token,
      [{ wallet: bob, change: amount }],
      { l1: true },
    );
    ...
    const [aliceReceipt, bobReceipt] = await Promise.all([
      aliceWithdrawalPromise,
      bobWithdrawalPromise,
    ]);
    ...
    await expect(alice.finalizeWithdrawal(aliceReceipt.hash)).toBeAccepted([
      aliceChange,
    ]);
    testMaster.reporter.debug("Finalized withdrawal for Alice");
    await expect(alice.finalizeWithdrawal(bobReceipt.hash)).toBeAccepted([
      bobChange,
    ]);
  ...
  test("Should execute a withdrawal with same parameters twice", async () => {
    ...
    const change1 = await shouldChangeTokenBalances(
      l1Token,
      [{ wallet: alice, change: amount }],
      { l1: true },
    );
    const change2 = await shouldChangeTokenBalances(
      l1Token,
      [{ wallet: alice, change: amount * 2n }],
      {
        l1: true,
      },
    );
    ...
    await expect(alice.finalizeWithdrawal(receipt1.hash)).toBeAccepted([
      change1,
    ]);
    testMaster.reporter.debug("Finalized withdrawal #1");
    await expect(alice.finalizeWithdrawal(receipt2.hash)).toBeAccepted([
      change2,
    ]);
  ...
  it("should reject transaction with huge gas limit", async () => {
    await expect(
      alice.sendTransaction({ to: alice.address, gasLimit: 2n ** 51n }),
  ...
  it("Create2Factory should work", async () => {
    ...
    const bytecode = await alice.provider.getCode(
    ...
    const deploymentTx = await (
      await contractFactory.create2(salt, hash, new Uint8Array(0))
    ).wait();
    ...
    expect(deployedAddress.deployedAddress.toLocaleLowerCase()).toEqual(
      correctCreate2Address.toLocaleLowerCase(),
    );
    expect(
      await alice.provider.getCode(deployedAddress.deployedAddress),
    ).toEqual(bytecode);
  });
  ...
  function bootloaderUtilsContract() {
    const BOOTLOADER_UTILS_ADDRESS =
  ...
  function eip712TxToTransactionData(tx: any): TransactionData {
    return {
      ...
      gasPerPubdataByteLimit:
        ...
        zksync.utils.DEFAULT_GAS_PER_PUBDATA_LIMIT,
      maxFeePerGas: tx.maxFeePerGas,
      maxPriorityFeePerGas: tx.maxPriorityFeePerGas,
