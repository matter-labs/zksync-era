...
use crate::{
    consensus::{self, sync_block_to_consensus_block},
    sync_layer::{
...
impl PostgresBlockStorage {
    ...
    pub async fn new(
        ...
        Self::ensure_genesis_block(ctx, &mut storage, genesis_block)
            .await
    ...
    async fn ensure_genesis_block(
        ...
        let block_number = u32::try_from(genesis_block.header.number.0)
            .context("Block number overflow for genesis block")?;
        let block = Self::sync_block(ctx, storage, MiniblockNumber(block_number))
            .await
            .wrap("sync_block();")?;
        let block = block
            .with_context(|| {
    ...
    async fn sync_block(
        ...
        Ok(ctx
            .wait(storage.sync_dal().sync_block(number, true))
            .await?
    ...
    async fn block(
        ...
        let Some(block) = Self::sync_block(ctx, storage, number)
            .await
...
impl BlockStore for PostgresBlockStorage {
    ...
    async fn block(&self, ctx: &ctx::Ctx, number: BlockNumber) -> ctx::Result<Option<FinalBlock>> {
        ...
        let mut storage = self.storage(ctx).await.wrap("storage()")?;
        Self::block(ctx, &mut storage, number)
            .await
            .wrap("Self::block()")
    }
