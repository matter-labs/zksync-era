...
for VERSION in $(seq -f "v1.4.%g" 0 1); do \
    mkdir -p $(pwd)/etc/zksolc-bin/$VERSION && \
...
mkdir -p $(pwd)/etc/zksolc-bin/vm-1.5.0-a167aa3 && \
    ...
    chmod +x $(pwd)/etc/zksolc-bin/vm-1.5.0-a167aa3/zksolc

# install zksolc 1.5.x
for VERSION in $(seq -f "v1.5.%g" 0 1); do \
    mkdir -p $(pwd)/etc/zksolc-bin/$VERSION && \
    wget https://github.com/matter-labs/zksolc-bin/raw/main/linux-amd64/zksolc-linux-amd64-musl-$VERSION -O /etc/zksolc-bin/$VERSION/zksolc && \
    chmod +x $(pwd)/etc/zksolc-bin/$VERSION/zksolc; \
    done
...
for VERSION in $(seq -f "v1.5.%g" 0 2); do \
    mkdir -p $(pwd)/etc/zkvyper-bin/$VERSION && \
    wget https://github.com/matter-labs/zkvyper-bin/raw/main/linux-amd64/zkvyper-linux-amd64-musl-$VERSION -O /etc/zkvyper-bin/$VERSION/zkvyper && \
    chmod +x $(pwd)/etc/zkvyper-bin/$VERSION/zkvyper; \
    done
