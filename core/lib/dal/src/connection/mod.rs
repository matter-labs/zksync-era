...
impl ConnectionPool {
    /// Establishes a pool of the connections to the database and creates a new `ConnectionPool` object.
    ///
    /// # Arguments
    ///
    /// - `pool_max_size` is the number of connections in pool. If not set, `DATABASE_POOL_SIZE` env var
    ///   is going to be used.
