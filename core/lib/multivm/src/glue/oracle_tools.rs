...
pub enum OracleTools<S: ReadStorage, H: HistoryMode> {
    M5(vm_m5::OracleTools<false, StorageView<S>>),
    M6(vm_m6::OracleTools<false, StorageView<S>, H::VmM6Mode>),
}
    ...
    S: ReadStorage + std::fmt::Debug,
{
    pub fn new(version: VmVersion, state: StoragePtr<StorageView<S>>, history: H) -> Self {
        match version {
            VmVersion::M5WithoutRefunds => {
                let oracle_tools = vm_m5::OracleTools::new(state, vm_m5::vm::MultiVMSubversion::V1);
                OracleTools::M5(oracle_tools)
            }
            VmVersion::M5WithRefunds => {
                let oracle_tools = vm_m5::OracleTools::new(state, vm_m5::vm::MultiVMSubversion::V2);
                OracleTools::M5(oracle_tools)
            }
    ...
    pub fn m6(&mut self) -> &mut vm_m6::OracleTools<false, StorageView<S>, H::VmM6Mode> {
    ...
    pub fn m5(&mut self) -> &mut vm_m5::OracleTools<false, StorageView<S>> {
