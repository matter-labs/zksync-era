...
use zksync_types::snapshots::{AllSnapshots, SnapshotBasicMetadata, SnapshotFullMetadata};
...
impl SnapshotsDal<'_, '_> {
    ...
    pub async fn get_snapshot(
        &mut self,
        l1_batch_number: L1BatchNumber,
    ) -> Result<Option<SnapshotFullMetadata>, sqlx::Error> {
        ...
        Ok(record.map(|r| SnapshotFullMetadata {
            l1_batch_number: L1BatchNumber(r.l1_batch_number as u32),
            generated_at: DateTime::<Utc>::from_naive_utc_and_offset(r.created_at, Utc),
...
mod tests {
    ...
    async fn adding_snapshot(pool: ConnectionPool) {
        ...
        assert_eq!(
            snapshot.unwrap().l1_batch_number,
