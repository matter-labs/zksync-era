...
use vm::{
    ...
    SystemEnv, VmExecutionResultAndLogs, VmTracer,
};
...
impl BatchExecutor {
    pub(super) fn run(
        ...
        vlog::info!("Starting executing batch #{:?}", &l1_batch_params.number);
    ...
    fn execute_tx_in_vm<S: ReadStorage>(
        ...
        let custom_tracers = if self.save_call_traces {
            vec![
                Box::new(CallTracer::new(call_tracer_result.clone(), HistoryEnabled))
                    as Box<dyn VmTracer<StorageView<S>, HistoryEnabled>>,
            ]
        } else {
            vec![]
        };
