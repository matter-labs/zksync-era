...
describe("Tests for the mempool behavior", () => {
  ...
  test("Should process shuffled nonces", async () => {
    ...
    const txs = nonceOffsets.map((offset) =>
      sendTxWithNonce(alice, startNonce + offset).then((tx) => tx.wait()),
    );
  ...
  test("Should discard too low nonce", async () => {
    const startNonce = await alice.getNonce();
    await expect(sendTxWithNonce(alice, startNonce - 1)).toBeRejected(
      "nonce too low.",
    );
  });
  ...
  test("Should discard too big nonce", async () => {
    ...
    const startNonce = await alice.getNonce();
    await expect(
      sendTxWithNonce(alice, startNonce + maxNonceAhead + 1),
    ).toBeRejected("nonce too high.");
  });
  ...
  test("Should replace the transaction", async () => {
    ...
    const tx2 = await sendTxWithNonce(alice, startNonce + 1);
    await expect(
      alice.provider.getTransaction(tx2.hash),
    ...
    const replacedTx2 = await sendTxWithNonce(
      alice,
      startNonce + 1,
      bob.address,
    );
    await expect(
      alice.provider.getTransaction(replacedTx2.hash),
    ).resolves.toMatchObject({
      nonce: startNonce + 1,
      to: bob.address,
    });
  ...
  test("Should reject a pre-sent transaction with not enough balance", async () => {
    ...
    await alice
      .sendTransaction({ to: poorBob.address, value: fund })
      .then((tx) => tx.wait());
