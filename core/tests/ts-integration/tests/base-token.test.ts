...
describe("base ERC20 contract checks", () => {
  ...
  beforeAll(async () => {
    ...
    isETHBasedChain = zksync.utils.isAddressEq(
      baseToken,
  ...
  test("Base token ratio is updated on L1", async () => {
    ...
    const numerator = Number(
      await zksyncContract.baseTokenGasPriceMultiplierNominator(),
    );
    const denominator = Number(
      await zksyncContract.baseTokenGasPriceMultiplierDenominator(),
    );
  ...
  test("Can perform a deposit", async () => {
    ...
    const initialL1Balance = await alice.getBalanceL1(
    ...
    const receipt = await alice
      ._providerL1()
      .getTransactionReceipt(depositHash);
    if (!receipt) {
  ...
  test("Can perform a transfer to self", async () => {
    ...
    const receipt = await alice
      ._providerL2()
      .getTransactionReceipt(transferTx.hash);
    const fee = receipt!.gasPrice * receipt!.gasUsed;
  ...
  test("Can perform a withdrawal", async () => {
    ...
    const withdrawalPromise = alice.withdraw({
      token: baseTokenDetails.l2Address,
      amount,
    });
    ...
    const receipt = await alice
      ._providerL2()
      .getTransactionReceipt(withdrawalTx.hash);
    const fee = receipt!.gasPrice * receipt!.gasUsed;
  ...
  test("Wrapped base token metadata", async () => {
    ...
    const wrappedBaseToken = new ethers.Contract(
      wethToken,
      zksync.utils.IERC20,
      alice,
    );
    ...
    expect(expectedWrappedBaseTokenSymbol).toEqual(
      await wrappedBaseToken.symbol(),
    );
  });
