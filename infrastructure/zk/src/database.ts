...
export async function dropMain() {
  ...
  await utils.spawn(
    `cargo sqlx database drop --database-url ${process.env.DATABASE_URL}`
  );
}
...
export async function setupMain() {
  ...
  await utils.spawn(
    `cargo sqlx database create --database-url ${process.env.DATABASE_URL}`
  );
  await utils.spawn(
    `cargo sqlx migrate run --database-url ${process.env.DATABASE_URL}`
  );
  if (process.env.DATABASE_URL!.startsWith(localDbUrl)) {
    await utils.spawn(
...
export async function waitMain(tries: number = 4) {
  for (let i = 0; i < tries; i++) {
    const result = await utils.allowFail(
      utils.exec(`pg_isready -d "${process.env.DATABASE_URL}"`)
    );
...
export async function checkSqlxDataMain() {
  process.chdir("core/lib/server_dal");
  await utils.spawn(
    `cargo sqlx prepare --check --database-url ${process.env.DATABASE_URL} -- --tests`
  );
  process.chdir(process.env.ZKSYNC_HOME as string);
}
...
export async function dropProver() {
  ...
  await utils.spawn(
    `cargo sqlx database drop --database-url ${process.env.DATABASE_PROVER_URL}`
  );
}
...
export async function setupProver() {
  ...
  await utils.spawn(
    `cargo sqlx database create --database-url ${process.env.DATABASE_PROVER_URL}`
  );
  await utils.spawn(
    `cargo sqlx migrate run --database-url ${process.env.DATABASE_PROVER_URL}`
  );
  if (process.env.DATABASE_PROVER_URL!.startsWith(localDbUrl)) {
    await utils.spawn(
...
export async function waitProver(tries: number = 4) {
  for (let i = 0; i < tries; i++) {
    const result = await utils.allowFail(
      utils.exec(`pg_isready -d '${process.env.DATABASE_PROVER_URL}'`)
    );
    if (result !== null) return; // null means failure
...
export async function checkSqlxDataProver() {
  process.chdir("prover/prover_dal");
  await utils.spawn(
    `cargo sqlx prepare --check --database-url ${process.env.DATABASE_PROVER_URL} -- --tests`
  );
  process.chdir(process.env.ZKSYNC_HOME as string);
}
...
command
  ...
  .action(checkSqlxDataMain);

command
  .command("prover-drop")
  .description("drop the prover database")
  .action(dropProver);
command
  .command("prover-migrate")
  .description("run prover migrations")
  .action(migrateProver);
command
...
command
  .command("prover-wait")
  .description("wait for prover database to get ready for interaction")
  .action(waitProver);
command
  .command("prover-reset")
  .description("reinitialize the prover database")
  .action(resetProver);
// command
