    ...
    fn from(x: &L1BatchWithMetadata) -> Self {
        Self {
            ...
            dependency_roots_rolling_hash: if x.header.system_logs.is_empty() {
                ...
                println!("case 5 {:?}", x.header.system_logs.iter().find(|log| log.0.key == MESSAGE_ROOT_ROLLING_HASH_KEY).unwrap());
                x.header.system_logs.iter().find(|log| log.0.key == MESSAGE_ROOT_ROLLING_HASH_KEY).unwrap().0.value
            },
