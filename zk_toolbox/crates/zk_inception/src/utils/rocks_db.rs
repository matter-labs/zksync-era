...
pub fn recreate_rocksdb_dirs(shell: &Shell, rocks_db_path: &Path, option: RocksDBDirOption) -> anyhow::Result<RocksDbs> {
    let state_keeper = rocks_db_path.join(option.prefix()).join(ROCKS_DB_STATE_KEEPER);
