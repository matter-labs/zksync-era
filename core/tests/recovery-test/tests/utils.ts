...
export function setSnapshotRecovery(
  pathToHome: string,
  chain: string,
): ReverseConfigPatch {
  return setPropertiesInGeneralConfig(pathToHome, chain, {
    "snapshot_recovery.enabled": true,
  });
}
...
function setPropertiesInGeneralConfig(
  ...
  let generalConfig = yaml.parseDocument(
    fs.readFileSync(generalConfigPath, "utf8"),
  );
  ...
  for (const [property, value] of Object.entries(properties)) {
    ...
    if (!generalConfig.hasIn(parentPath)) {
      throw new Error(
        `Config at ${generalConfigPath} doesn't contain parent for property ${property}`,
      );
    }
  ...
  if (isReverse) {
    console.log(
      `Reversed properties in ${generalConfigPath} to previous values:`,
      properties,
    );
  } else {
    console.log(
      `Created reverse patch for ${generalConfigPath}:`,
      reverseProperties,
    );
  }
