...
impl MiniblockSealCommand {
    pub async fn seal(&self, storage: &mut StorageProcessor<'_, Server>) {
        self.seal_inner(storage, false).await;
    }

    async fn insert_transactions(&self, transaction: &mut StorageProcessor<'_, Server>) {
    ...
    async fn seal_inner(&self, storage: &mut StorageProcessor<'_, Server>, is_fictive: bool) {
