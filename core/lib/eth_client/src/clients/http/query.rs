...
use zksync_web3_decl::{
    client::{Client, DynClient, EthereumLikeNetwork, ForEthereumLikeNetwork, MockClient, L1},
...
impl<T: EthereumLikeNetwork> EthInterface for Box<DynClient<T>> {
...
#[async_trait::async_trait]
impl<T: EthereumLikeNetwork> EthInterface for DynClient<T> {
    async fn fetch_chain_id(&self) -> EnrichedClientResult<SLChainId> {
        COUNTERS.call[&(Method::ChainId, self.component())].inc();
        let latency = LATENCIES.direct[&Method::ChainId].start();
        let raw_chain_id = self.chain_id().rpc_context("chain_id").await?;
...
#[async_trait::async_trait]
impl<T: EthereumLikeNetwork> EthInterface for Client<T> {
    async fn fetch_chain_id(&self) -> EnrichedClientResult<SLChainId> {
        COUNTERS.call[&(Method::ChainId, self.component())].inc();
        let latency = LATENCIES.direct[&Method::ChainId].start();
        let raw_chain_id = self.chain_id().rpc_context("chain_id").await?;
...
#[async_trait::async_trait]
impl<T: EthereumLikeNetwork> EthInterface for MockClient<T> {
    async fn fetch_chain_id(&self) -> EnrichedClientResult<SLChainId> {
        COUNTERS.call[&(Method::ChainId, self.component())].inc();
        let latency = LATENCIES.direct[&Method::ChainId].start();
        let raw_chain_id = self.chain_id().rpc_context("chain_id").await?;
