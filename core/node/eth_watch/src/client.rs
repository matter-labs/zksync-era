...
use zksync_contracts::{
    ...
    l1_asset_router_contract, l2_message_root, state_transition_manager_contract,
    verifier_contract, wrapped_base_token_store_contract,
};
...
pub trait EthClient: 'static + fmt::Debug + Send + Sync {
    ...
    /// Call the `getL2UpgradeTxData(bridgehub, chainId, existingTxData)` view on the upgrade
    /// facet's `initAddress` to obtain the per-chain-rewritten `l2ProtocolUpgradeTx.data`.
    ///
    /// `SettlementLayerV31UpgradeBase.upgrade()` performs the same rewrite inside the L1
    /// diamond-cut delegatecall; calling the view off-chain ensures the tx we inject into the
    /// mempool matches what L1 wrote into the priority queue. Callers gate on minor version —
    /// pre-v31 init contracts do not expose this function.
    ...
    Box<DynClient<Net>>: EthInterface + GetLogsClient,
{
    ...
    async fn get_l2_upgrade_tx_data(
        ...
        // `getL2UpgradeTxData(address bridgehub, uint256 chainId, bytes existingTxData)` is a
        // plain `view` function; `SettlementLayerV31UpgradeBase.upgrade()` calls it from inside the
        // diamond-cut delegatecall to replace the placeholder `additionalForceDeploymentsData` in
        // the L2 upgrade tx. Calling it directly against the upgrade facet's deployed address here
        // yields the same bytes that L1 wrote into the priority queue, so the state keeper can
        // execute the upgrade tx without us re-implementing the per-chain encoding in Rust.
        //
        // NOTE: this works for Era chains because `EraSettlementLayerV31Upgrade` compares the
        // placeholder's `isZKsyncOS = false` against `s.zksyncOS`, which reads as `false` from the
        // uninitialised impl contract's storage. For a ZKsyncOS chain the placeholder would carry
        // `isZKsyncOS = true` and the same call would revert with `UnexpectedZKsyncOSFlag`.
        // zksync-era only serves Era chains, so this is fine; a ZKsyncOS server needs a different
        // approach (see zksync-os-server PR #1186).
        ...
        CallFunctionArgs::new(
            "getL2UpgradeTxData",
            (bridgehub, U256::from(self.l2_chain_id.as_u64()), existing_tx_data),
        )
