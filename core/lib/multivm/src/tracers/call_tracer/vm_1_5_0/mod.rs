...
use crate::{
    ...
    vm_1_5_0::{BootloaderState, HistoryMode, SimpleMemory, VmTracer, ZkSyncVmState},
};
    ...
    fn after_execution(
        ...
        match data.opcode.variant.opcode {
            ...
            Opcode::FarCall(far_call) => {
                ...
                self.handle_far_call_op_code_vm_1_5_0(state, memory, &mut current_call);
                self.push_call_and_update_stats(current_call, 0);
            }
            Opcode::Ret(ret_code) => {
                self.handle_ret_op_code_vm_1_5_0(state, memory, ret_code);
            }
            _ => {}
        };
    }
...
impl CallTracer {
    fn handle_far_call_op_code_vm_1_5_0<H: HistoryMode>(
        &mut self,
        state: VmLocalStateData<'_>,
        memory: &SimpleMemory<H>,
    ...
    fn save_output_vm_1_5_0<H: HistoryMode>(
        &mut self,
        state: VmLocalStateData<'_>,
        memory: &SimpleMemory<H>,
    ...
    fn handle_ret_op_code_vm_1_5_0<H: HistoryMode>(
        &mut self,
        state: VmLocalStateData<'_>,
        memory: &SimpleMemory<H>,
        ret_opcode: RetOpcode,
        ...
        self.save_output_vm_1_5_0(state, memory, ret_opcode, &mut current_call.farcall);
