...
pub fn get_test_data() -> V26TestData {
    V26TestData {
        l1_chain_id: 1.into(),
        l1_shared_bridge_address: Address::from_str("abacabac00000000000000000000000000000001").unwrap(),
        ...
        l2_legacy_shared_bridge_address: Address::from_str("d316600b6009f94ab776e97f1fc985bc18b4e535").unwrap(),
        l1_aliased_shared_bridge: Address::from_str("bcbdabac00000000000000000000000000001112").unwrap(),
    }
}

fn get_test_v26_path(name: &str) -> PathBuf {
    let core_path = Workspace::locate().core();
    Path::new(&core_path).join(&format!("lib/multivm/src/versions/testonly/v26_utils_outputs/{name}"))
}
...
pub fn trivial_test_storage_logs() -> HashMap<StorageKey, H256> {
    let x: Vec<_> = serde_json::from_str(
        &std::fs::read_to_string(&get_test_v26_path("simple-test.json"))
            .unwrap(),
    )
    .unwrap();
    x.into_iter().collect()
...
pub fn post_bridging_test_storage_logs() -> HashMap<StorageKey, H256> {
    let x: Vec<_> = serde_json::from_str(
        &std::fs::read_to_string(&get_test_v26_path("post-bridging.json"))
            .unwrap(),
    )
    .unwrap();
    x.into_iter().collect()
...
pub fn post_registration_test_storage_logs() -> HashMap<StorageKey, H256> {
    let x: Vec<_> = serde_json::from_str(
        &std::fs::read_to_string(&get_test_v26_path("post-registration.json"))
            .unwrap(),
    )
    .unwrap();
    x.into_iter().collect()
