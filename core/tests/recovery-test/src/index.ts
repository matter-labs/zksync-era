...
export async function getExternalNodeHealth(url: string) {
  try {
    const response: HealthCheckResponse = await fetch(url).then((response) =>
      response.json(),
    );
    return response;
  } catch (e) {
...
export async function executeCommandWithLogs(
  command: string,
  logsPath: string,
) {
...
export class NodeProcess {
  ...
  static async spawn(
    ...
    const logs =
      typeof logsFile === "string" ? await fs.open(logsFile, "a") : logsFile;
    let componentsArr =
      deploymentMode === "Validium"
        ? [withDAFetcher(components)]
        : [components];
...
export class FundedWallet {
  static async create(
    mainNode: zksync.Provider,
    eth: ethers.Provider,
  ): Promise<FundedWallet> {
    if (!process.env.MASTER_WALLET_PK) {
      const testConfigPath = path.join(
        process.env.ZKSYNC_HOME!,
        `etc/test_config/constant/eth.json`,
      );
      const ethTestConfig = JSON.parse(
        await fs.readFile(testConfigPath, { encoding: "utf-8" }),
      );
      const mnemonic = ethers.Mnemonic.fromPhrase(ethTestConfig.test_mnemonic);
      const walletHD = ethers.HDNodeWallet.fromMnemonic(
        mnemonic,
        "m/44'/60'/0'/0/0",
      );
  ...
  async ensureIsFunded() {
    ...
    expect(
      l1Balance >= minExpectedBalance,
      "L1 balance of funded wallet is too small",
    ).to.be.true;

    const baseTokenAddress = await this.wallet.getBaseToken();
    const isETHBasedChain =
  ...
  async generateL1Batch(): Promise<number> {
    ...
    console.log(
      "Generated a transaction from funded wallet",
      transactionResponse,
    );
