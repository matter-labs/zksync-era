...
export class TestMaster {
  ...
  private constructor(file: string) {
    ...
    if (!contextStr) {
      throw new Error(
        "Test context was not initialized; unable to load context environment variable",
      );
    }
    ...
    this.l1Provider = new EthersRetryProvider(
      this.env.l1NodeUrl,
      "L1",
      this.reporter,
    );
    ...
    this.mainWallet = new RetryableWallet(
      suiteWalletPK,
      this.l2Provider,
      this.l1Provider,
    );
  ...
  newEmptyAccount(): zksync.Wallet {
    ...
    const newWallet = new RetryableWallet(
      randomPK,
      this.l2Provider,
      this.l1Provider,
    );
  ...
  ethersProvider(layer: "L1" | "L2"): EthersRetryProvider {
    ...
    const provider = new EthersRetryProvider(
      { url, timeout: 120_000 },
      layer,
      this.reporter,
    );
  ...
  async deinitialize() {
    try {
      const promises = await claimEtherBack(
        this.subAccounts,
