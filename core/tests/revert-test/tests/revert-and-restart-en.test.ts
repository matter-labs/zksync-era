...
} from "zksync-ethers/build/typechain";
...
describe("Block reverting test", function () {
  ...
  before("initialize test", async () => {
    ...
    mainNodeSpawner = new NodeSpawner(
      pathToHome,
      mainLogs,
      chainName,
      mainNodeSpawnOptions,
    );
    ...
    extNodeSpawner = new NodeSpawner(
      pathToHome,
      extLogs,
      chainName,
      extNodeSpawnOptions,
    ...
    settlementLayerMainContract = gatewayInfo
      ? IZkSyncHyperchain__factory.connect(
  ...
  step("Seal L1 batch", async () => {
    depositL1BatchNumber = await extNode.createBatchWithDeposit(
      alice.address,
      depositAmount,
    );
  });
  ...
  step("wait for the new batch to be committed", async () => {
    batchesCommittedBeforeRevert = await waitToCommitBatchesWithoutExecution(
  ...
  step("wait until last deposit is re-executed", async () => {
    ...
    let tryCount = 0;
    while (
      (balanceBefore = await alice.getBalance()) !== 2n * depositAmount &&
      tryCount < 30
    ) {
    ...
    assert(
      balanceBefore === 2n * depositAmount,
      "Incorrect balance after revert",
    );
  });
  ...
  step("execute transaction after revert", async () => {
    ...
    assert(
      balanceAfter === depositAmount * 3n,
      "Incorrect balance after another deposit",
    );
  });
