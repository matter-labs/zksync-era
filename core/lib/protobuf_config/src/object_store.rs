...
impl ProtoRepr for proto::ObjectStore {
    ...
    fn read(&self) -> anyhow::Result<Self::Type> {
        Ok(Self::Type {
            bucket_base_url: required(&self.bucket_base_url).context("bucket_base_url")?.clone(),
            mode: required(&self.mode).and_then(|x|Ok(proto::ObjectStoreMode::try_from(*x)?)).context("mode")?.parse(),
            file_backed_base_path: required(&self.file_backed_base_path).context("file_backed_base_path")?.clone(),
            gcs_credential_file_path: required(&self.gcs_credential_file_path).context("gcs_credential_file_path")?.clone(),
            max_retries: required(&self.max_retries).and_then(|x|Ok((*x).try_into()?)).context("max_retries")?,
        })
    }
