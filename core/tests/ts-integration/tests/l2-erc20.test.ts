...
describe("L2 native ERC20 contract checks", () => {
  ...
  beforeAll(async () => {
    ...
    l2NativeTokenVault = new zksync.Contract(
    ...
    const tokenApprovalTx = await aliceErc20.approve(
      L2_NATIVE_TOKEN_VAULT_ADDRESS,
      100n,
    );
  ...
  test("check weth", async () => {
    ...
    const wethabi = await readContract(
      "../../../contracts/l2-contracts/zkout",
      "L2WETH",
    ).abi;
    ...
    try {
      ...
      expect(err.toString()).toContain(
        ethers.concat(["0x06439c6b", ethers.zeroPadBytes("0x", 12), weth]),
      );
    }
  ...
  test("Can perform a withdrawal", async () => {
    ...
    tokenDetails.l1Address =
      await l1NativeTokenVault.tokenAddress(zkTokenAssetId);
    const balanceAfterBridging = await alice.getBalanceL1(
      tokenDetails.l1Address,
    );
