...
pub fn commit_gas_count_for_l1_batch(l1_batch: &L1BatchWithMetadata) -> u32 {
    let base_cost = l1_batch_base_cost(AggregatedActionType::Commit);
    let total_messages_len: u32 = l1_batch
        .header
        .l2_to_l1_messages
        .iter()
        .map(|message| message.len() as u32)
        .sum();
    let total_factory_deps_len: u32 = l1_batch
        .factory_deps
        .iter()
        .map(|factory_dep| factory_dep.len() as u32)
        .sum();
    let additional_calldata_bytes = l1_batch.metadata.initial_writes_compressed.len() as u32
        + l1_batch.metadata.repeated_writes_compressed.len() as u32
        + l1_batch.metadata.l2_l1_messages_compressed.len() as u32
        + total_messages_len
