...
async function setupConfiguration() {
  ...
  const questions: BasePromptOptions[] = [
    {
      message:
        "Do you want to configure a new chain or use an existing configuration?",
      name: "config",
      type: "select",
...
async function setHyperchainMetadata() {
  ...
  const questions: BasePromptOptions[] = [
    ...
    {
      message:
        "What is your hyperchain id? Make sure this is not used by other chains.",
      name: "chainId",
      type: "numeral",
      required: true,
    },
  ...
  if (results.l1Chain !== BaseNetwork.LOCALHOST) {
    ...
    if (connectionsResults.generateKeys === GENERATE_KEYS) {
      ...
      const keyQuestions: BasePromptOptions[] = [
        {
          message:
            "Private key of the L1 Deployer (the one that deploys the contracts)",
          name: "deployerKey",
          type: "password",
          required: true,
        },
        {
          message:
            "Private key of the L1 Governor (the one that can upgrade the contracts)",
          name: "governorKey",
          type: "password",
          required: true,
        },
        {
          message:
            "Private key of the L1 ETH Operator (the one that rolls up the batches)",
          name: "ethOperator",
          type: "password",
          required: true,
        },
  ...
  if (
    results.l1Chain !== BaseNetwork.LOCALHOST_CUSTOM &&
    results.l1Chain !== BaseNetwork.LOCALHOST
  ) {
    const verifyQuestions: BasePromptOptions[] = [
      {
        message:
          "Do You want to verify your L1 contracts? You will need a etherscan API key for it.",
        name: "verify",
        type: "confirm",
      },
    ];
  ...
  wrapEnvModify(
    ...
    ethOperator.privateKey
  );
  wrapEnvModify(
    "ETH_SENDER_SENDER_OPERATOR_COMMIT_ETH_ADDR",
    ethOperator.address
  );
  ...
  wrapEnvModify(
    "ETH_SENDER_SENDER_L1_BATCH_MIN_AGE_BEFORE_EXECUTE_SECONDS",
    "20"
  );
...
async function setupHyperchainProver() {
  ...
  const proverQuestions: BasePromptOptions[] = [
    {
      ...
      required: true,
      choices: [
        ProverTypeOption.NONE,
        ProverTypeOption.CPU,
        ProverTypeOption.GPU,
      ],
    },
  ];
  ...
  switch (proverType) {
    ...
    default:
      await setupProver(
        proverType === ProverTypeOption.CPU ? ProverType.CPU : ProverType.GPU
      );
  }
}
    ...
    if (!process.env.DEPLOY_TEST_TOKENS) {
      ...
      if (!baseWethToken) {
        const wethQuestions = [
          {
            message:
              "What is the address of the Wrapped ETH on the base chain?",
            name: "l1Weth",
            type: "input",
            required: true,
          },
        ...
        if (
          fs.existsSync(`/etc/tokens/${getEnv(process.env.ZKSYNC_ENV!)}.json`)
        ) {
          tokens.push({
            name: "Wrapped Ether",
            symbol: "WETH",
...
async function startServer() {
  ...
  const NO =
    "Not right now (you can now configure prover, generate docker files, or just run the server later)";
  ...
  if (results.start === NO) {
    ...
    components = ((await enquirer.prompt(componentQuestions)) as any)
      .components;
  } else {
...
async function checkReadinessToDeploy() {
  ...
  const governor = new ethers.Wallet(
    process.env.GOVERNOR_PRIVATE_KEY!,
    provider
  );
  const ethOperator = new ethers.Wallet(
    process.env.ETH_SENDER_SENDER_OPERATOR_PRIVATE_KEY!,
    provider
  );
  ...
  async function checkAllWalletBalances(): Promise<Boolean> {
    ...
    checkPromises.push(
      checkBalance(ethOperator, ethers.utils.parseEther("0.5"))
    );
...
async function _generateDockerImages(_orgName?: string) {
  ...
  const templateFileName =
    "./etc/hyperchains/docker-compose-hyperchain-template.hbs";
  const templateString =
    fs.existsSync(templateFileName) &&
    fs.readFileSync(templateFileName).toString().trim();
...
async function configDemoHyperchain(cmd: Command) {
  ...
  fs.existsSync("/etc/hyperchains/hyperchain-demo.yml") &&
    fs.unlinkSync("/etc/hyperchains/hyperchain-demo.yml");
...
function printReadme() {
  ...
  console.log(
    announce(
      "Please follow these steps/commands to get your hyperchain tailored to your (and your users) needs.\n"
    )
  );

  console.log(
    `${chalk.bgBlueBright("zk stack init")} ${chalk.blueBright(
      "- Wizard for hyperchain creation/configuration"
    )}`
  );
  console.log(
...
initHyperchainCommand
  ...
  .option(
    "--custom-docker-org <value>",
    "Custom organization name for the docker images"
  )
...
initHyperchainCommand
  ...
  .option(
    "--skip-env-setup",
    "Run env setup automatically (pull docker containers, etc)"
  )
  .description(
    "Spin up a demo hyperchain with default settings for testing purposes"
  )
