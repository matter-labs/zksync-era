...
use crate::{
    commands::prover::{
        args::setup_database::{SetupDatabaseArgs, SetupDatabaseArgsFinal},
        gcs::get_project_ids,
    },
    ...
    messages::{
        ...
        MSG_SETUP_DATABASE_HELP, MSG_SETUP_DATABASE_PROMPT, MSG_SETUP_KEY_PATH_PROMPT,
    },
};
...
pub struct ProverInitArgs {
    ...
    #[clap(
        long, help = MSG_SETUP_DATABASE_HELP, default_missing_value = "true", num_args = 0..=1
    )]
    pub setup_database: Option<bool>,
    #[clap(flatten)]
    #[serde(flatten)]
    pub database_config: SetupDatabaseArgs,
...
pub struct ProverInitArgsFinal {
    ...
    pub setup_database: bool,
    pub database_config: Option<SetupDatabaseArgsFinal>,
}
...
impl ProverInitArgs {
    pub(crate) fn fill_values_with_prompt(
        ...
        let setup_database = self
            .setup_database
            .unwrap_or_else(|| PromptConfirm::new(MSG_SETUP_DATABASE_PROMPT).ask());

        let database_config = if setup_database {
            Some(self.fill_database_values_with_prompt())
        } else {
            None
        };
    ...
    fn fill_database_values_with_prompt(&self) -> SetupDatabaseArgsFinal {
        self.database_config
            .clone()
            .get_database_config_with_prompt()
    }
}
