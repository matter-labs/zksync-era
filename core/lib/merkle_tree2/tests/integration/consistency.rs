...
use zksync_merkle_tree2::{Database, MerkleTree, RocksDBWrapper};
use zksync_storage::{db, rocksdb::WriteBatch};
...
fn five_thousand_angry_monkeys_vs_merkle_tree() {
    ...
    let (_, patch) = MerkleTree::new(&db).extend(kvs);
    ...
    for _ in 0..ITER_COUNT {
        ...
        let db = RocksDBWrapper::from(raw_db);
        ...
        let cf = raw_db.cf_merkle_tree_handle(db::MerkleTreeColumnFamily::Tree);
        let mut reverse_batch = WriteBatch::default();
