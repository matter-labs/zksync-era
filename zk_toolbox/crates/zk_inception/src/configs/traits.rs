...
pub trait ReadConfig: DeserializeOwned + Clone {
    fn read(path: impl AsRef<Path>) -> anyhow::Result<Self> {
        ...
        match path.as_ref().extension().and_then(|ext| ext.to_str()) {
            Some("yaml") | Some("yml") => serde_yaml::from_reader(file).with_context(error_context),
            Some("toml") => {
                let mut buffer = String::new();
                std::io::BufReader::new(file).read_to_string(&mut buffer)?;
                toml::from_str(&buffer).with_context(error_context)
            }
            Some("json") => serde_json::from_reader(file).with_context(error_context),
            _ => bail!(format!(
