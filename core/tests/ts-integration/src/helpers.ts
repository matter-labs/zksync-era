 ...
 */
export async function anyTransaction(
 ...
 */
export async function waitForNewL1Batch(
  wallet: zksync.Wallet,
  ...
  for (let i = 0; i < MAX_ATTEMPTS; i++) {
    ...
    if (
      ...
      txResponse.maxFeePerGas < gasPrice
    ) {
      txResponse = await wallet
        ...
        .catch((e) => {
          ...
          if (<Error>e.message.match(/Not enough gas/)) {
            ...
            <Error>e.message.match(/max fee per gas less than block base fee/)
          ) {
            console.log(
    ...
    txReceipt = await wallet.provider
      .waitForTransaction(txResponse.hash, 1, 3000)
      .catch((e) => {
        if (ethers.isError(e, "TIMEOUT")) {
          console.log(
            `Transaction timed out, potentially gas price went up (attempt ${i + 1}/${MAX_ATTEMPTS})`,
          );
          return null;
        } else if (
          ethers.isError(e, "UNKNOWN_ERROR") &&
          e.message.match(/Not enough gas/)
        ) {
          console.log(
            `Transaction did not have enough gas, likely gas price went up (attempt ${i + 1}/${MAX_ATTEMPTS})`,
          );
  ...
  while (
    !(await wallet.provider.getTransactionReceipt(txReceipt.hash))
      ?.l1BatchNumber
  ) {
...
export async function getDeploymentNonce(
  ...
  const nonceHolder = new zksync.Contract(
    zksync.utils.NONCE_HOLDER_ADDRESS,
    zksync.utils.NONCE_HOLDER_ABI,
    provider,
  );
