...
use zksync_contracts::{zksync_contract, BaseSystemContractsHashes};
...
#[db_test]
async fn test_parse_system_contracts_hashes(connection_pool: ConnectionPool) {
    ...
    let original_correct_form_data = Token::Array(vec![
        ...
        Token::Tuple(vec![
            // should contain 2 Tokens: first is Bool and second is Bytes
            Token::Bool(true),
            Token::Bytes(vec![
        ...
        Token::Tuple(vec![
            Token::Bool(true),
            Token::Bytes(vec![
                40, 72, 156, 45, 219, 103, 54, 150, 36, 37, 58, 97, 81, 255, 186, 33, 35, 20, 195,
                77, 19, 182, 23, 65, 145, 9, 223, 123, 242, 64, 225, 149,
            ]),
        ]),
    ]);
    ...
    assert!(tester
        .aggregator
        .parse_base_system_contracts_hashes(original_correct_form_data)
        .is_ok());
    ...
    for wrong_data_instance in original_wrong_form_data {
        assert_matches!(
            tester
                .aggregator
                .parse_base_system_contracts_hashes(wrong_data_instance.clone()),
...
#[db_test]
async fn test_generate_calldata(connection_pool: ConnectionPool) {
    let tester = EthSenderTester::new(connection_pool, vec![100; 100], false).await;

    // actual result
    let result = tester.aggregator.generate_calldata_for_multicall();
    ...
    let get_bootloader_hash_call = Multicall3Call {
        target: tester.aggregator.main_zksync_contract_address,
        allow_failure: false,
...
#[db_test]
async fn get_l1_base_system_contracts_hashes(connection_pool: ConnectionPool) {
    let mut tester = EthSenderTester::new(connection_pool, vec![100; 100], false).await;
    let base_system_contracts_hashes = tester
        .aggregator
        .get_l1_base_system_contracts_hashes(&tester.gateway)
        .await;
    assert!(base_system_contracts_hashes.is_ok());
}
