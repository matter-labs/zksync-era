...
CREATE TABLE IF NOT EXISTS leaf_aggregation_witness_jobs (
    ...
    protocol_version INTEGER CONSTRAINT leaf_aggregation_witness_jobs_prover_protocol_version_fkey REFERENCES prover_protocol_versions (id)
);
...
CREATE TABLE IF NOT EXISTS scheduler_witness_jobs (
    ...
    is_blob_cleaned BOOLEAN DEFAULT FALSE NOT NULL,
    protocol_version INTEGER CONSTRAINT scheduler_witness_jobs_prover_protocol_version_fkey REFERENCES prover_protocol_versions (id)
);
...
CREATE TABLE IF NOT EXISTS witness_inputs (
    ...
    is_blob_cleaned BOOLEAN DEFAULT false NOT NULL,
    protocol_version INTEGER CONSTRAINT witness_inputs_prover_protocol_version_fkey REFERENCES prover_protocol_versions (id)
);
...
CREATE TABLE IF NOT EXISTS node_aggregation_witness_jobs (
    ...
    is_blob_cleaned BOOLEAN DEFAULT false NOT NULL,
    protocol_version INTEGER CONSTRAINT node_aggregation_witness_jobs_prover_protocol_version_fkey REFERENCES prover_protocol_versions (id)
);
...
CREATE TABLE IF NOT EXISTS prover_jobs (
    ...
    proccesed_by TEXT,
    is_blob_cleaned BOOLEAN DEFAULT FALSE NOT NULL,
    protocol_version INTEGER CONSTRAINT prover_jobs_prover_protocol_version_fkey REFERENCES prover_protocol_versions (id)
) WITH (
...
CREATE INDEX IF NOT EXISTS ix_prover_jobs_circuits_0_2 ON public.prover_jobs USING btree (aggregation_round DESC, l1_batch_number, id) INCLUDE (protocol_version)
WHERE
    (
        (status = 'queued' :: text)
        AND (
...
CREATE INDEX IF NOT EXISTS ix_prover_jobs_circuits_1_2 ON public.prover_jobs USING btree (aggregation_round DESC, l1_batch_number, id) INCLUDE (protocol_version)
WHERE
    (
        (status = 'queued' :: text)
        AND (
...
CREATE INDEX IF NOT EXISTS ix_prover_jobs_circuits_2_2 ON public.prover_jobs USING btree (aggregation_round DESC, l1_batch_number, id) INCLUDE (protocol_version)
WHERE
    (
        (status = 'queued' :: text)
        AND (
...
CREATE INDEX IF NOT EXISTS ix_prover_jobs_circuits_3_2 ON public.prover_jobs USING btree (aggregation_round DESC, l1_batch_number, id) INCLUDE (protocol_version)
WHERE
    (
        (status = 'queued' :: text)
        AND (
...
CREATE INDEX IF NOT EXISTS ix_prover_jobs_circuits_4_2 ON public.prover_jobs USING btree (aggregation_round DESC, l1_batch_number, id) INCLUDE (protocol_version)
WHERE
    (
        (status = 'queued' :: text)
        AND (
...
CREATE INDEX IF NOT EXISTS ix_prover_jobs_circuits_5_2 ON public.prover_jobs USING btree (aggregation_round DESC, l1_batch_number, id) INCLUDE (protocol_version)
WHERE
    (
        (status = 'queued' :: text)
        AND (
...
CREATE INDEX IF NOT EXISTS ix_prover_jobs_circuits_6_2 ON public.prover_jobs USING btree (aggregation_round DESC, l1_batch_number, id) INCLUDE (protocol_version)
WHERE
    (
        (status = 'queued' :: text)
        AND (
...
CREATE INDEX IF NOT EXISTS ix_prover_jobs_circuits_7_2 ON public.prover_jobs USING btree (aggregation_round DESC, l1_batch_number, id) INCLUDE (protocol_version)
WHERE
    (
        (status = 'queued' :: text)
        AND (
...
CREATE INDEX IF NOT EXISTS ix_prover_jobs_circuits_8_2 ON public.prover_jobs USING btree (aggregation_round DESC, l1_batch_number, id) INCLUDE (protocol_version)
WHERE
    (
        (status = 'queued' :: text)
        AND (
...
CREATE INDEX IF NOT EXISTS ix_prover_jobs_circuits_9_2 ON public.prover_jobs USING btree (aggregation_round DESC, l1_batch_number, id) INCLUDE (protocol_version)
WHERE
    (
        (status = 'queued' :: text)
