...
export class Tester {
  ...
  async fundSyncWallet() {
    const baseTokenAddress =
      await this.syncWallet.provider.getBaseTokenContractAddress();
    if (!(baseTokenAddress === zksync.utils.ETH_ADDRESS_IN_CONTRACTS)) {
      ...
      await (
        await l1Erc20Contract.mint(this.ethWallet.address, BASE_ERC20_TO_MINT)
      ).wait();
    }
  }
  ...
  async fundedWallet(
    ...
    const newWallet = new zksync.Wallet(
      newWalletHD.privateKey,
      this.web3Provider,
      this.ethProvider,
    );
    ...
    expect(
      ...
      "Insufficient eth balance to create funded wallet",
    ).to.be.true;
    ...
    expect(
      ...
      "Insufficient token balance to create funded wallet",
    ).to.be.true;
