//! Airbender cycle-estimator calibration features.
//!
//! These are the *model inputs* an Airbender guest cycle-count estimate needs —
//! opcode-family counts plus precompile/decommit/storage complexity. They live in
//! `vm_interface` (rather than in the `multivm` tracer that fills them) so they can
//! ride in [`VmExecutionStatistics`](super::VmExecutionStatistics), be accumulated
//! across a batch by the state keeper, and be read by a seal criterion — all
//! without those layers depending on `multivm`.
//!
//! The tracer that fills a [`FeatureVector`] and the calibrated cost model that
//! turns it into a cycle estimate both live in
//! `zksync_multivm::tracers::cycle_estimator`.
...
/// `zksync_multivm::tracers::cycle_estimator::CostModel::unpriced_used` so callers
/// can fail safe.
///
...
impl FeatureVector {
    ...
    /// Merge every count from `other` into `self` (element-wise addition). Used to
    /// accumulate per-transaction feature vectors into a running batch vector.
