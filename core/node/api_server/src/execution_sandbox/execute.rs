...
use zk_os_forward_system::run::{
    BatchContext, ExecutionOutput, ExecutionResult as ZkOSExecutionResult, StorageCommitment,
    TxOutput,
};
    ...
    pub async fn execute_in_sandbox_zkos(
        ...
        match result {
            Ok(Ok(tx_output)) => match tx_output.execution_result {
                ZkOSExecutionResult::Success(ExecutionOutput::Call(data)) => Ok(data),
                ZkOSExecutionResult::Success(ExecutionOutput::Create(data, _)) => Ok(data),
                ZkOSExecutionResult::Revert(res) => anyhow::bail!("revert: {:?}", res),
            },
            Ok(Err(invalid)) => {
