    ...
    tokio::task::spawn_blocking(move || {
        ...
        let storage: Box<dyn ReadStorage> = match config.data_source {
            ...
            BasicWitnessGeneratorDataSource::FromPostgresShadowBlob => {
                let witness_block_state: WitnessBlockState = rt_handle.block_on(async {
                    object_store
                        .get(input.block_number)
                        .await
                        .expect("could not access object_store value for witness storage")
                });
                let connection = rt_handle.block_on(connection_pool.access_storage());
                ...
                let checked_storage =
                    Box::new(WitnessStorage::new(input.block_number, witness_block_state));
