...
const ArtifactBridgeHub = readContract(
	'../../../contracts/l1-contracts/artifacts/contracts/bridgehub',
	'Bridgehub'
);
const ArtifactNativeTokenVault = readContract(
...
describe("Interop checks", () => {
    ...
    beforeAll(async () => {
      ...
      l1_wallet = new zksync.Wallet(mainAccount.privateKey, l1_provider, l1_provider);
      ...
      interop1_rich_wallet = new zksync.Wallet(l1_wallet.privateKey, interop1_provider, l1_provider);
      ...
      interop2_rich_wallet = new zksync.Wallet(l1_wallet.privateKey, interop2_provider, l1_provider);
      ...
      console.log(`[rich wallet] l1_wallet address: ${l1_wallet.address} with ${ethers.formatEther(await l1_provider.getBalance(l1_wallet.address))} ETH`);
      console.log("--------------------");
      ...
      await (await interop1_rich_wallet.deposit({
      ...
      await (await interop2_rich_wallet.deposit({
        token: ETH_ADDRESS_IN_CONTRACTS,
        amount: fundAmount,
      ...
      const interop1_tokenA_contract_deployment = await deployContract(interop1_wallet, ArtifactMintableERC20, ["Token A", "AA", 18]);
      ...
      const interop2_tokenB_contract_deployment = await deployContract(interop2_wallet, ArtifactMintableERC20, ["Token B", "BB", 18]);
      ...
      tokenB_details.addresses.interop1 = await interop1_nativeTokenVault_contract.tokenAddress(tokenB_details.assetId);
      ...
      const interop2_swap_contract_deployment = await deployContract(interop2_wallet, ArtifactSwap, [tokenA_details.addresses.interop2, tokenB_details.addresses.interop2]);
      const interop2_swap_contract_address = await interop2_swap_contract_deployment.getAddress();
      interop2_swap_contract = new zksync.Contract(
        interop2_swap_contract_address,
        ArtifactSwap.abi,
        interop2_wallet,
      );
      ...
      await (await interop2_tokenB_contract.mint(await interop2_swap_contract.getAddress(), ethers.parseEther("1000"))).wait();
      console.log(`Swap contract token B balance: ${ethers.formatEther(await getTokenBalance({
      ...
      await (await interop1_tokenA_contract_deployment.mint(interop1_wallet.address, ethers.parseEther("500"))).wait();
    ...
    test("Can transfer token A from Interop1 to Interop2", async () => {
      ...
      console.log(`Test wallet token A Interop 1 balance before transfer: ${ethers.formatEther(interop1_tokenA_balance_before)} AA`);
      ...
      console.log(`Test wallet token A Interop 1 balance after transfer: ${ethers.formatEther(interop1_tokenA_balance_after)} AA`);
      ...
      console.log(`Test wallet token A Interop 2 balance after transfer: ${ethers.formatEther(interop2_tokenA_balance_after)} AA`);
    });
    ...
    test("Can perform cross chain swap", async () => {
      ...
      console.log(`Test wallet token B Interop2 balance before swap: ${ethers.formatEther(interop2_tokenB_balance_before)} BB`);
      ...
      console.log(`Test wallet token B Interop2 balance after swap: ${ethers.formatEther(interop2_tokenB_balance_after)} BB`);
    });
    ...
    test("Can transfer token B from Interop2 to Interop1", async () => {
      ...
      tokenB_details.addresses.interop1 = await interop1_nativeTokenVault_contract.tokenAddress(tokenB_details.assetId);
      if (tokenB_details.addresses.interop1 === ethers.ZeroHash) throw new Error("Token B resolves to zero address on Interop1");
      ...
      console.log(`Test wallet token B Interop1 balance after transfer: ${ethers.formatEther(interop1_tokenB_balance_after)} BB`);
    });
    ...
    async function from_interop1_approveSwapAllowance() {
      ...
      const tx = await from_interop1_requestL2TransactionDirect(mintValue, tokenA_details.addresses.interop2, BigInt(0), l2Calldata);
      await tx.wait();
    ...
    async function from_interop1_swap_a_to_b() {
      ...
      const tx = await from_interop1_requestL2TransactionDirect(mintValue, await interop2_swap_contract.getAddress(), BigInt(0), l2Calldata);
      await readAndBroadcastInteropTx(tx.hash, interop1_provider, interop2_provider);
    }
    ...
    async function readAndBroadcastInteropTx(
      ...
      let {
        ...
        l2TxNumberInBlock,
        message,
      } = { l1BatchNumber: 0, l2TxNumberInBlock: 0, message: "" };
      ...
      try {
        ...
        const sender_chain_utilityWallet = new zksync.Wallet(zksync.Wallet.createRandom().privateKey, sender_chain_provider);
        const {
        ...
        console.error("Error reading interop message:", e);
        return;
      }
      ...
      const log = {
        ...
        key: ethers.keccak256(
          ethers.AbiCoder.defaultAbiCoder().encode(["address"], [interop1_wallet.address]),
        ),
        value: ethers.keccak256(
          ethers.AbiCoder.defaultAbiCoder().encode(["bytes"], [message]),
        ),
      };
  
      const leafHash = ethers.keccak256(
        ethers.AbiCoder.defaultAbiCoder().encode([L2_LOG_STRING], [log]),
      );
      ...
      const interopTx = {
        ...
        customData: {
          ...
          toMint: (xl2Input.reserved[0].toString(16).length % 2 == 0 ? '0x' : '0x0') + xl2Input.reserved[0].toString(16),
          refundRecipient: interop1_wallet.address,
        },
      ...
      const interopTxAsCanonicalTx = {
        txType: BigInt(INTEROP_TX_TYPE),
        from: interopTx.from,
        to: interopTx.to,
        gasLimit: interopTx.gasLimit,
        gasPerPubdataByteLimit: BigInt(50000),
    ...
    async function getTokenBalance({ provider, tokenAddress, address }: { provider: zksync.Provider, tokenAddress: string, address: string }) {
