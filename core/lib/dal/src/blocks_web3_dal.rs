...
impl BlocksWeb3Dal<'_, '_> {
    ...
    pub async fn get_block_tx_count(
        &mut self,
        block_id: api::BlockId,
    ) -> Result<Option<(MiniblockNumber, U256)>, SqlxError> {
        let query = format!(
    ...
    /// Pending batch is not in the database but its timestamp is equal to the timestamp of its first miniblock.
    pub async fn get_expected_l1_batch_timestamp(
        &mut self,
        l1_batch_number: L1BatchNumber,
    ) -> Result<Option<u64>, SqlxError> {
        let timestamp = sqlx::query!(
            "SELECT timestamp as \"timestamp!\" FROM ( \
                (SELECT number, timestamp FROM miniblocks WHERE l1_batch_number = $1 ORDER BY number LIMIT 1) \
                    UNION \
                (SELECT number, timestamp FROM miniblocks WHERE l1_batch_number IS NULL ORDER BY number LIMIT 1) \
            ) as subquery ORDER BY number LIMIT 1",
    ...
    ) -> Result<Option<api::BlockDetails>, SqlxError> {
        {
    ...
    pub async fn get_l1_batch_details(
        &mut self,
        l1_batch_number: L1BatchNumber,
    ) -> Result<Option<api::L1BatchDetails>, SqlxError> {
        {
