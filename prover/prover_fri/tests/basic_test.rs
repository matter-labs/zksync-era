...
use zksync_prover_fri::prover_job_processor::ProverJob;
use zksync_prover_fri_utils::CircuitWrapper;
use zksync_vk_setup_data_server_fri::{generate_cpu_base_layer_setup_data, ProverServiceDataKey};
...
async fn prover_and_assert_base_layer(
    ...
    let circuit_wrapper = object_store.get(blob_key).await.expect("circuit missing");
    let circuit = match &circuit_wrapper {
        CircuitWrapper::Base(base) => base.clone(),
        CircuitWrapper::Recursive(_) => {
            panic!("Expected base layer circuit")
        }
    };
    ...
    let artifacts = Prover::prove(
        prover_job,
        Arc::new(FriProverConfig::from_env()),
        setup_data,
    );
...
async fn test_base_layer_sha256_proof_gen() {
    prover_and_assert_base_layer(1293714, 6, L1BatchNumber(114499), 479).await;
}
