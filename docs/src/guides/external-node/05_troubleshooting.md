...
The Node tries to follow the fail-fast principle: if an anomaly is discovered, instead of attempting state
...
The Node is supposed to start with an applied DB dump. If you see any genesis-related errors, it probably means
the Node was started without an applied dump.
