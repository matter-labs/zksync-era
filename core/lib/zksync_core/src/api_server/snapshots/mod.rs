...
use zksync_config::configs::api::SnapshotsApiConfig;
use zksync_dal::connection::ConnectionPool;
use zksync_utils::panic_notify::{spawn_panic_handler, ThreadPanicNotify};

use api::SnapshotsRestApi;

pub mod api;

fn start_server(api: SnapshotsRestApi, bind_to: SocketAddr, threads: usize) -> Server {
    HttpServer::new(move || {
        let api = api.clone();
        App::new()
            .wrap(
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            actix_rt::System::new().block_on(async move {
                ...
                let api = SnapshotsRestApi::new(replica_connection_pool);
                ...
                actix_rt::spawn(async move {
                    if stop_receiver.changed().await.is_ok() {
                        ...
                        tracing::info!("Stop signal received, snapshots API is shutting down");
                    }
                });
                server.await.expect("Snapshots API crashed");
            });
        })
