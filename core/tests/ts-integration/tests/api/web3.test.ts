...
describe("web3 API compatibility tests", () => {
  ...
  test("Should test block/transaction web3 methods", async () => {
    ...
    const blockWithTxsByNumber = await alice.provider.getBlock(
      blockNumber,
      true,
    );
    ...
    const blockByNumberFull = await alice.provider.send(
      "eth_getBlockByNumber",
      [blockNumberHex, false],
    );
    expect(blockByNumberFull.logsBloom).toEqual(
      expect.stringMatching(HEX_VALUE_REGEX),
    );
    expect(blockByNumberFull.logsBloom.length).toEqual(514);
    expect(
      blockByNumberFull.logsBloom != ethers.zeroPadValue("0x00", 256),
    ).toBeTruthy();
    ...
    for (const tx of blockWithTxsByNumber.prefetchedTransactions) {
      const receipt = await alice.provider.send("eth_getTransactionReceipt", [
        tx.hash,
      ]);
    ...
    let receipts = await alice.provider.send("eth_getBlockReceipts", [
      blockNumberHex,
    ]);
    ...
    const txCountByNumber = await alice.provider.send(
      "eth_getBlockTransactionCountByNumber",
      [blockNumberHex],
    );
    expect(parseInt(txCountByNumber, 16)).toEqual(
      blockWithTxsByNumber.prefetchedTransactions.length,
    );

    // eth_getBlockTransactionCountByHash
    const txCountByHash = await alice.provider.send(
      "eth_getBlockTransactionCountByHash",
      [blockHash],
    );
    expect(parseInt(txCountByHash, 16)).toEqual(
      blockWithTxsByNumber.prefetchedTransactions.length,
    );

    // eth_getTransactionByBlockNumberAndIndex
    const txByBlockNumberAndIndex = await alice.provider.send(
      "eth_getTransactionByBlockNumberAndIndex",
      [blockNumberHex, "0x0"],
    );

    // eth_getTransactionByBlockHashAndIndex
    const txByBlockHashAndIndex = await alice.provider.send(
      "eth_getTransactionByBlockHashAndIndex",
      [blockHash, "0x0"],
    );
    ...
    const txByHash = await alice.provider.send("eth_getTransactionByHash", [
      txByBlockNumberAndIndex.hash,
    ]);
  ...
  test("Should test storage web3 methods", async () => {
    ...
    // eth_getCode
    const code = await alice.provider.getCode(
    ...
    const codeKey =
      ...
      (await counterContract.getAddress()).substring(2);
    const codeHash = await alice.provider.getStorage(
      accCodeStorageAddress,
      codeKey,
    );
  ...
  test("Should check the network version", async () => {
    ...
    await expect(alice.provider.send("net_version", [])).resolves.toMatch(
      chainId.toString(),
    );
  });
  ...
  test.each([
    ...
    "Should test bogus web3 methods (%s)",
    async (method: string, input: string[], output: any) => {
  ...
  test("Should test bogus web3 methods (eth_getUncleCountByBlockHash)", async () => {
    ...
    await expect(
      alice.provider.send("eth_getUncleCountByBlockHash", [firstBlockHash]),
    ).resolves.toEqual("0x0");
  });
  ...
  test("Should test web3 response extensions", async () => {
    ...
    const blockWithTransactions = await alice.provider.getBlock(
      receipt!.blockNumber,
      true,
    );
  ...
  test("Should check transactions from API / EIP1559 tx", async () => {
    ...
    const eip1559ApiReceipt = await alice.provider.getTransaction(
      eip1559Tx.hash,
    );
  ...
  test("Should test getFilterChanges for pending transactions", async () => {
    ...
    const filterId = await alice.provider.send(
      "eth_newPendingTransactionFilter",
      [],
    );
    let changes: string[] = await alice.provider.send("eth_getFilterChanges", [
      filterId,
    ]);
    ...
    testMaster.reporter.debug(
      "Sent new transactions with hashes",
      remainingHashes,
    );
  ...
  test("Should test pub-sub API: blocks", async () => {
    ...
    let wsProvider = new ethers.WebSocketProvider(
      testMaster.environment().wsL2NodeUrl,
    );
  ...
  test("Should test transaction status", async () => {
    ...
    status = await alice.provider.getTransactionStatus(sentTx.hash);
    expect(
      status == types.TransactionStatus.Processing ||
  ...
  test("Should check API accepts JSON RPC requests with additional fields", async () => {
    ...
    await expect(
      fetch(testMaster.environment().l2NodeUrl, req).then((response) =>
        response.json(),
      ),
  ...
  test("Should check API returns correct block for every tag", async () => {
    const earliestBlock = await alice.provider.send("eth_getBlockByNumber", [
      "earliest",
      true,
    ]);
    expect(+earliestBlock.number!).toEqual(0);
    const committedBlock = await alice.provider.send("eth_getBlockByNumber", [
      "committed",
      true,
    ]);
    expect(+committedBlock.number!).toEqual(expect.any(Number));
    const finalizedBlock = await alice.provider.send("eth_getBlockByNumber", [
      "finalized",
      true,
    ]);
    expect(+finalizedBlock.number!).toEqual(expect.any(Number));
    const latestBlock = await alice.provider.send("eth_getBlockByNumber", [
      "latest",
      true,
    ]);
    expect(+latestBlock.number!).toEqual(expect.any(Number));
    const l1CommittedBlock = await alice.provider.send("eth_getBlockByNumber", [
      "l1_committed",
      true,
    ]);
    expect(+l1CommittedBlock.number!).toEqual(expect.any(Number));
    const pendingBlock = await alice.provider.send("eth_getBlockByNumber", [
      "pending",
      true,
    ]);
  ...
  test("Should check sendRawTransaction returns GasPerPubDataLimitZero with 0 gas_per_pubdata_limit", async () => {
    ...
    await expect(alice.sendTransaction(tx)).toBeRejected(
      "gas per pub data limit is zero",
    );
  });
  ...
  test("Should check getLogs endpoint works properly with blockHash", async () => {
    ...
    const getLogsByHash = (
      await alice.provider.getLogs({ blockHash: latestBlock.hash || undefined })
    ).map((x) => {
      return new zksync.types.Log({ ...x, l1BatchNumber: 0 }, alice.provider); // Set bogus value.
    });
  ...
  test("Should check eth_feeHistory", async () => {
    ...
    for (let i = 0; i < 2; i += 1) {
      const expectedBaseFee = (
        await alice.provider.getBlock(receipt.blockNumber - 1 + i)
      ).baseFeePerGas;
  ...
  test("Should check zks_getProtocolVersion endpoint", async () => {
    const latestProtocolVersion = await alice.provider.send(
    ...
    const exactProtocolVersion = await alice.provider.send(
      "zks_getProtocolVersion",
  ...
  test("Should check transaction signature for legacy transaction type", async () => {
    ...
    const serializedLegacyTxReq =
      ethers.Transaction.from(legacyTxReq).unsignedSerialized;
    ...
    const signerAddr = ethers.recoverAddress(
      ethers.keccak256(serializedLegacyTxReq),
      txFromApi.signature,
    );
    expect(signerAddr).toEqual(alice.address);

    const expectedV = 35n + BigInt(chainId) * 2n;
    const actualV = ethers.Signature.getChainIdV(
      chainId,
      txFromApi.signature.v,
    );
  ...
  test("Should check transaction signature for EIP1559 transaction type", async () => {
    ...
    const tx_handle =
      await alice.provider.broadcastTransaction(signedEip1559TxReq);
    await tx_handle.wait();

    const txFromApi = await alice.provider.getTransaction(tx_handle.hash);

    const serializedEip1559TxReq =
      ethers.Transaction.from(eip1559TxReq).unsignedSerialized;

    // check that API returns correct signature values for the given transaction
    // by invoking recoverAddress() method with the serialized transaction and signature values
    const signerAddr = ethers.recoverAddress(
      ethers.keccak256(serializedEip1559TxReq),
      txFromApi.signature,
    );
    expect(signerAddr).toEqual(alice.address);
  ...
  describe("Storage override", () => {
    ...
    test("Should be able to estimate_gas overriding contract code", async () => {
      ...
      const incrementFunctionData = contract2.interface.encodeFunctionData(
        "increment",
        [1],
      );
    ...
    test("Should estimate gas by overriding state with State", async () => {
      ...
      const sumValuesFunctionData = contract.interface.encodeFunctionData(
        "sumValues",
        [],
      );
    ...
    test("Should estimate gas by overriding state with StateDiff", async () => {
      const contract = await deployContract(alice, contracts.stateOverride, []);
      const contractAddress = await contract.getAddress();
      const incrementFunctionData = contract.interface.encodeFunctionData(
        "increment",
        [1],
      );
  ...
  test("Should check inner-outer contract address in the receipt of the deploy tx", async () => {
    ...
    const expectedAddress = zksync.utils.createAddress(
      alice.address,
      deploymentNonce,
    );
    ...
    const outerContract = await deployContract(
      alice,
      contracts.outer,
      [1],
      undefined,
      outerContractOverrides,
    );
    const contract = await outerContract.waitForDeployment();

    const deployedBytecode = await alice.provider.getCode(
  ...
  async request(req: { method: string; params?: any[] }) {
    ...
    switch (method) {
      ...
      case "eth_sendTransaction":
        ...
        const response =
          await this.wallet.provider.broadcastTransaction(signed);
      ...
      default:
        ...
        if (method.startsWith("zks")) {
          throw new Error(
            "zks namespace methods are not forwarded by metamask",
          );
        }
