...
describe("Upgrade test", function () {
  ...
  before("Create test wallet", async () => {
    ...
    bytecodeSupplier =
      contractsConfig.ecosystem_contracts.l1_bytecodes_supplier_addr;
    ...
    l1AdminGovWallet = new ethers.Wallet(
      chainWalletConfig.governor.private_key,
      alice._providerL1(),
    );
    ...
    if (
      chainWalletConfig.governor.private_key ==
      ecosystemWalletConfig.governor.private_key
    ) {
      ecosystemGovWallet = l1AdminGovWallet;
    } else {
      ecosystemGovWallet = new ethers.Wallet(
        ecosystemWalletConfig.governor.private_key,
        alice._providerL1(),
      );
    }
  ...
  step("Run server and execute some transactions", async () => {
    ...
    const baseToken =
      await tester.syncWallet.provider.getBaseTokenContractAddress();
    ...
    if (
      ...
      await (
        await tester.syncWallet.approveERC20(baseToken, ethers.MaxUint256)
      ).wait();
      await mintToAddress(
        baseToken,
        tester.ethWallet,
    ...
    while (
      (await tester.web3Provider.getL1BatchNumber()) <= initialL1BatchNumber
    ) {
      await utils.sleep(1);
    }
    ...
    while (
      (await tester.web3Provider.getL1BatchNumber()) <=
      initialL1BatchNumber + 1
    ) {
      await utils.sleep(1);
    }

    const balance = await alice.getBalance();
    expect(balance === depositAmount * 2n, "Incorrect balance after deposits")
      .to.be.true;
    ...
    if (process.env.CHECK_EN_URL) {
      ...
      expect(
        enBalance === balance,
        "Failed to update the balance on EN after deposit",
      ).to.be.true;
    }
  ...
  step("Publish bytecodes", async () => {
    ...
    defaultAccountHash = ethers.hexlify(
      zksync.utils.hashBytecode(defaultAACode),
    );
    evmEmulatorHash = ethers.hexlify(
      zksync.utils.hashBytecode(evmEmulatorCode),
    );

    let nonce = await tester.ethWallet.getNonce();
    nonce += await publishBytecode(
      tester.ethWallet,
      bytecodeSupplier,
      bootloaderCode,
      nonce,
  ...
  step("Schedule governance call", async () => {
    const forceDeployment: ForceDeployment = {
      bytecodeHash: ethers.hexlify(
        zksync.utils.hashBytecode(forceDeployBytecode),
      ),
    ...
    const delegateCalldata =
      contracts.l2ForceDeployUpgraderAbi.encodeFunctionData("forceDeploy", [
        [forceDeployment],
      ]);
    ...
    const { stmUpgradeData, chainUpgradeCalldata, setTimestampCalldata } =
    ...
    await sendGovernanceOperation(
      pauseMigrationCalldata.scheduleTransparentOperation,
      0,
      null,
    );
    ...
    await sendGovernanceOperation(
      stmUpgradeData.scheduleTransparentOperation,
      0,
      null,
    );
  ...
  step("Check bootloader is updated on L2", async () => {
    ...
    const batchDetails = await alice.provider.getL1BatchDetails(
      receipt.l1BatchNumber!,
    );
    expect(batchDetails.baseSystemContractsHashes.bootloader).to.eq(
      bootloaderHash,
    );
  });
  ...
  step("Finalize upgrade on the target chain", async () => {
    ...
    while (
      (await alice.provider.getL1BatchDetails(l1BatchNumber))
        .baseSystemContractsHashes.bootloader == bootloaderHash
    ) {
      l1BatchNumber -= 1;
    }
  ...
  async function sendGovernanceOperation(
    ...
    console.log(
      `Sent governance operation, tx_hash=${transaction.hash}, nonce=${transaction.nonce}`,
    );
  ...
  async function chainAdminSetTimestamp(data: string) {
    ...
    console.log(
      `Sent chain admin operation, tx_hash=${transaction.hash}, nonce=${transaction.nonce}`,
    );
  ...
  async function sendChainAdminOperation(
    ...
    const executeMulticallData =
      l1ChainAdminContract.interface.encodeFunctionData("multicall", [
        [call],
        true,
      ]);
  ...
  async function deployDefaultUpgradeImpl(
    governorWallet: ethers.Wallet,
    bridgehubAddr: string,
  ): Promise<string> {
    const bytecodePath = gatewayInfo
      ? pathToHome +
        "/contracts/l1-contracts/zkout/DefaultUpgrade.sol/DefaultUpgrade.json"
      : pathToHome +
        "/contracts/l1-contracts/out/DefaultUpgrade.sol/DefaultUpgrade.json";

    let bytecode = JSON.parse(readFileSync(bytecodePath).toString()).bytecode
      .object;
    ...
    if (gatewayInfo) {
      ...
      const governanceData = await prepareGovernanceCalldata(
        CONTRACT_DEPLOYER_ADDRESS,
        CONTRACT_DEPLOYER.encodeFunctionData("create2", [
          salt,
          bytecodeHash,
          "0x",
        ]),
      ...
      await sendGovernanceOperation(
        governanceData.scheduleTransparentOperation,
        0,
        null,
      );
...
function readCode(newPaths: string[], legacyPath?: string): string {
  ...
  if (legacyPath) {
    ...
    if (path.endsWith(".zbin")) {
      ...
      return "0x".concat(
        legacyContent.bytecode?.object || legacyContent.bytecode,
      );
    }
  } else {
...
async function checkedRandomTransfer(
  ...
  expect(
    receiverBalanceAfter === amount,
    "Failed updated the balance of the receiver",
  ).to.be.true;

  const spentAmount = txReceipt.gasUsed * transferHandle.gasPrice! + amount;
  expect(
    senderBalanceAfter + spentAmount >= senderBalanceBefore,
    "Failed to update the balance of the sender",
  ).to.be.true;
  ...
  if (process.env.CHECK_EN_URL) {
    ...
    expect(
      enSenderBalance === senderBalanceAfter,
      "Failed to update the balance of the sender on EN",
    ).to.be.true;
  }
...
async function prepareUpgradeCalldata(
  ...
  const settlementLayerCTMAddress =
    await settlementLayerDiamondProxy.getChainTypeManager();

  const oldProtocolVersion = Number(
    await settlementLayerDiamondProxy.getProtocolVersion(),
  );
  const newProtocolVersion = addToProtocolVersion(oldProtocolVersion, 1, 1);

  params.l2ProtocolUpgradeTx.nonce ??= BigInt(
    unpackNumberSemVer(newProtocolVersion)[1],
  );
  const upgradeInitData = contracts.l1DefaultUpgradeAbi.encodeFunctionData(
    "upgrade",
    [
      [
  ...
  const stmUpgradeCalldata = contracts.chainTypeManager.encodeFunctionData(
  ...
  const chainUpgradeCalldata = contracts.adminFacetAbi.encodeFunctionData(
    "upgradeChainFromVersion",
    [oldProtocolVersion, upgradeParam],
  );

  // Set timestamp for upgrade on a specific chain under this STM.
  const setTimestampCalldata = contracts.chainAdminAbi.encodeFunctionData(
    "setUpgradeTimestamp",
    [newProtocolVersion, params.upgradeTimestamp],
  );
...
async function prepareGovernanceCalldata(
  ...
  const scheduleTransparentOperation =
    contracts.governanceAbi.encodeFunctionData("scheduleTransparent", [
      governanceOperation,
      0, // delay
    ]);

  // Get transaction data of the `execute`
  const executeOperation = contracts.governanceAbi.encodeFunctionData(
    "execute",
    [governanceOperation],
  );
...
async function composeL1ToL2Call(
  ...
  const encodedData = zksync.utils.BRIDGEHUB_ABI.encodeFunctionData(
...
export function addToProtocolVersion(
  packedProtocolVersion: number,
  minor: number,
  patch: number,
) {
  const [major, minorVersion, patchVersion] = unpackNumberSemVer(
...
async function waitForPriorityOps(
  receipt: ethers.TransactionReceipt,
  providerForPriorityOp: zksync.Provider,
) {
