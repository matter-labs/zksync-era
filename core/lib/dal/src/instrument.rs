...
use zksync_db_connection::{BasicStorageProcessor, StorageProcessorTags};
    ...
    O: Send + Unpin + for<'r> FromRow<'r, PgRow>,
{
    ...
    pub async fn fetch_all(self, storage: &mut BasicStorageProcessor<'_>) -> sqlx::Result<Vec<O>> {
        let (conn, tags) = storage.conn_and_tags();
    ...
    A: 'q + Send + IntoArguments<'q, Postgres>,
{
    ...
    pub async fn fetch_one(self, storage: &mut BasicStorageProcessor<'_>) -> sqlx::Result<O> {
        let (conn, tags) = storage.conn_and_tags();
