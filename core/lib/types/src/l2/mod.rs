...
impl From<L2TxEvm> for TransactionRequest {
...
impl From<L2TxEvm> for Transaction {
    fn from(tx: L2TxEvm) -> Self {
        let L2TxEvm {
            execute,
            common_data,
...
impl From<L2TxEvm> for api::Transaction {
    fn from(tx: L2TxEvm) -> Self {
        let tx_type = tx.common_data.transaction_type as u32;
        let (v, r, s) =
            if let Ok(sig) = PackedEthSignature::deserialize_packed(&tx.common_data.signature) {
...
impl TryFrom<Transaction> for L2TxEvm {
    type Error = &'static str;
    ...
    fn try_from(value: Transaction) -> Result<Self, Self::Error> {
        ...
        match common_data {
            ExecuteTransactionCommon::L1(_) => Err("Cannot convert L1Tx to L2Tx"),
            ExecuteTransactionCommon::L2(common_data) => Ok(L2TxEvm {
                execute,
                common_data,
                received_timestamp_ms,
                raw_bytes,
...
impl EIP712TypedStructure for L2TxEvm {
    const TYPE_NAME: &'static str = "Transaction";
