...
describe('EVM equivalence contract', () => {
    ...
    describe('ERC20', () => {
        ...
        beforeEach(async () => {
            ...
            await evmToken.deploymentTransaction()?.wait(1, 10000);
    ...
    describe('Uniswap-v2', () => {
        ...
        describe('EVM ERC20', () => {
            ...
            let evmToken3: ethers.BaseContract;
            let evmToken4: ethers.BaseContract;


            beforeEach(async () => {
                const erc20Factory = getEVMContractFactory(alice, artifacts.erc20);
                evmToken1 = await erc20Factory.deploy();
                await evmToken1.deploymentTransaction()?.wait();
                evmToken2 = await erc20Factory.deploy();
                await evmToken2.deploymentTransaction()?.wait();
                evmToken3 = await erc20Factory.deploy();
                await evmToken3.deploymentTransaction()?.wait();
                evmToken4 = await erc20Factory.deploy();
                await evmToken4.deploymentTransaction()?.wait();
                ...
                const evmPairReceipt2 = await (
                    await evmUniswapFactory.getFunction('createPair')(evmToken2.getAddress(), evmToken3.getAddress())
                ).wait();

                const nativePairReceipt = await (
                    await nativeUniswapFactory.getFunction('createPair')(evmToken3.getAddress(), evmToken4.getAddress())
                ).wait();

                const nativePairReceipt2 = await (
                    await nativeUniswapFactory.getFunction('createPair')(evmToken2.getAddress(), evmToken3.getAddress())
                ).wait();
                ...
                const token3IsFirst = (await nativeUniswapPair.getFunction('token0')()).toString() === evmToken3.getAddress();
                if (!token3IsFirst) {
                    [evmToken3, evmToken4] = [evmToken4, evmToken3];
                }
                ...
                await (await evmToken3.getFunction('transfer')(nativeUniswapPair.getAddress(), 100000)).wait();
                await (await evmToken4.getFunction('transfer')(nativeUniswapPair.getAddress(), 100000)).wait();
                ...
                if (logGasCosts && !deployLogged) {
                    ...
                    let logString = 'EVM ERC20 \n';
                    logString += 'Uniswap Factory native deploy gas: ' + native_transanction_receipt.gasUsed + "\n";
                    logString += 'Uniswap Factory evm deploy gas: ' + evm_transanction_receipt.gasUsed + "\n";
                    logString += '\n';
                    logString += 'Uniswap Pair native create gas: ' + nativePairReceipt.gasUsed + "\n";
                    logString += 'Uniswap Pair evm create gas: ' + evmPairReceipt.gasUsed + "\n";
                    logString += '\n';
                    logString += 'Uniswap Pair native create second time gas: ' + nativePairReceipt2.gasUsed + "\n";
                    logString += 'Uniswap Pair evm create second time gas: ' + evmPairReceipt2.gasUsed + "\n";
                    logString += '\n';
            ...
            test('mint, swap, and burn should work', async () => {
                ...
                await (await evmToken3.getFunction('transfer')(nativeUniswapPair.getAddress(), 10000)).wait();
                ...
                if (logGasCosts) {
                    let logString = 'EVM ERC20 \n';
                    logString += 'UniswapV2Pair native mint gas: ' + nativeMintReceipt.gasUsed + "\n";
                    logString += 'UniswapV2Pair evm mint gas: ' + evmMintReceipt.gasUsed + "\n";
                    logString += '\n';
                    logString += 'UniswapV2Pair native swap gas: ' + nativeSwapReceipt.gasUsed + "\n";
                    logString += 'UniswapV2Pair evm swap gas: ' + evmSwapReceipt.gasUsed + "\n";
                    logString += '\n';
                    logString += 'UniswapV2Pair native burn gas: ' + nativeBurnReceipt.gasUsed + "\n";
                    logString += 'UniswapV2Pair evm burn gas: ' + evmBurnReceipt.gasUsed + "\n";
                    logString += '\n';
                    console.log(logString);
                }
                ...
                expect(Number((await evmToken3.getFunction('balanceOf')(alice.address)).toString())).toBeGreaterThanOrEqual(
                    990000
                );
                expect(Number((await evmToken4.getFunction('balanceOf')(alice.address)).toString())).toBeGreaterThanOrEqual(
                    990000
                );
            });
        ...
        describe.only('Native ERC20', () => {
            let eraToken1: zksync.Contract;
            let eraToken2: zksync.Contract;
            let eraToken3: zksync.Contract;
            let eraToken4: zksync.Contract;


            beforeEach(async () => {
                const erc20Factory = getEVMContractFactory(alice, artifacts.erc20);
                eraToken1 = await deployContract(alice, contracts.erc20, []);
                eraToken2 = await deployContract(alice, contracts.erc20, []);
                eraToken3 = await deployContract(alice, contracts.erc20, []);
                eraToken4 = await deployContract(alice, contracts.erc20, []);
                ...
                const evmPairReceipt = await (
                    await evmUniswapFactory.getFunction('createPair')(eraToken1.getAddress(), eraToken2.getAddress())
                ).wait();

                const evmPairReceipt2 = await (
                    await evmUniswapFactory.getFunction('createPair')(eraToken2.getAddress(), eraToken3.getAddress())
                ).wait();

                const nativePairReceipt = await (
                    await nativeUniswapFactory.getFunction('createPair')(eraToken3.getAddress(), eraToken4.getAddress())
                ).wait();

                const nativePairReceipt2 = await (
                    await nativeUniswapFactory.getFunction('createPair')(eraToken2.getAddress(), eraToken3.getAddress())
                ).wait();
                ...
                const token1IsFirst = (await evmUniswapPair.getFunction('token0')()).toString() === eraToken1.getAddress();
                if (!token1IsFirst) {
                    [eraToken1, eraToken2] = [eraToken2, eraToken1];
                }

                const token3IsFirst = (await nativeUniswapPair.getFunction('token0')()).toString() === eraToken3.getAddress();
                if (!token3IsFirst) {
                    [eraToken3, eraToken4] = [eraToken4, eraToken3];
                }
                await (await eraToken1.getFunction('transfer')(evmUniswapPair.getAddress(), 100000)).wait();
                await (await eraToken2.getFunction('transfer')(evmUniswapPair.getAddress(), 100000)).wait();
                await (await eraToken3.getFunction('transfer')(nativeUniswapPair.getAddress(), 100000)).wait();
                await (await eraToken4.getFunction('transfer')(nativeUniswapPair.getAddress(), 100000)).wait();
            ...
            test('mint, swap, and burn should work', async () => {
                ...
                await (await eraToken1.getFunction('transfer')(evmUniswapPair.getAddress(), 10000)).wait();
                await (await eraToken3.getFunction('transfer')(nativeUniswapPair.getAddress(), 10000)).wait();
                const evmSwapReceipt = await (
                    await evmUniswapPair.getFunction('swap')(0, 5000, alice.address, '0x')
                ).wait();
                const nativeSwapReceipt = await (
                ...
                expect(Number((await eraToken1.getFunction('balanceOf')(alice.address)).toString())).toBeGreaterThanOrEqual(
                    990000
                );
                expect(Number((await eraToken2.getFunction('balanceOf')(alice.address)).toString())).toBeGreaterThanOrEqual(
                    990000
                );

                expect(Number((await eraToken3.getFunction('balanceOf')(alice.address)).toString())).toBeGreaterThanOrEqual(
                    990000
                );
                expect(Number((await eraToken4.getFunction('balanceOf')(alice.address)).toString())).toBeGreaterThanOrEqual(
                    990000
                );
            });
