...
export class TestContextOwner {
  ...
  requiredL1ETHPerAccount() {
    return isLocalHost(this.env.network)
      ? L1_EXTENDED_TESTS_ETH_PER_ACCOUNT
      : L1_DEFAULT_ETH_PER_ACCOUNT;
  }

  // Returns the required amount of L2 ETH
  requiredL2ETHPerAccount() {
    return isLocalHost(this.env.network)
      ? L2_EXTENDED_TESTS_ETH_PER_ACCOUNT
      : L2_DEFAULT_ETH_PER_ACCOUNT;
  }
  ...
  private async cancelPendingTxs() {
    ...
    this.reporter.debug(
      `Latest nonce is ${latestNonce}, pending nonce is ${pendingNonce}`,
    );
    ...
    this.reporter.debug(
      `Max nonce is ${latestNonce}, pending nonce is ${pendingNonce}`,
    );
    ...
    if (cancellationTxs.length > 0) {
      ...
      this.reporter.message(
        `Canceled ${cancellationTxs.length} pending transactions`,
      );
    }
    this.reporter.finishAction();
  }
  ...
  private async cancelAllowances() {
    ...
    await tx.wait();
    this.reporter.debug(
      `Sent ERC20 cancel approve transaction. Hash: ${tx.hash}, nonce ${tx.nonce}`,
    );
    ...
    if (baseToken != zksync.utils.ETH_ADDRESS_IN_CONTRACTS) {
      ...
      await tx.wait();
      this.reporter.debug(
        `Sent base token cancel approve transaction. Hash: ${tx.hash}, nonce ${tx.nonce}`,
      );
    }
  ...
  private async prepareWallets(): Promise<TestWallets> {
    ...
    const baseTokenAddress = await bridgehubContract.baseToken(
      this.env.l2ChainId,
    );
  ...
  private async ensureBalances(accountsAmount: bigint): Promise<bigint> {
    ...
    this.reporter.message(
      `Operator address is ${this.mainEthersWallet.address}`,
    );
    ...
    this.reporter.message(
      `Operator balance on L2 is ${ethers.formatEther(actualL2ETHAmount)} ETH`,
    );

    // We may have enough funds in L2. If that's the case, no need to deposit more than required.
    const l2ETHAmountToDeposit =
      requiredL2ETHAmount > actualL2ETHAmount
        ? requiredL2ETHAmount - actualL2ETHAmount
        : 0n;

    const requiredL1ETHAmount =
      this.requiredL1ETHPerAccount() * accountsAmount + l2ETHAmountToDeposit;
    const actualL1ETHAmount = await this.mainSyncWallet.getBalanceL1();
    this.reporter.message(
      `Operator balance on L1 is ${ethers.formatEther(actualL1ETHAmount)} ETH`,
    );
  ...
  private createTestWallets(suites: string[]): TestWallets {
    ...
    this.reporter.debug(
      `Test wallets: ${JSON.stringify(wallets, undefined, 2)}`,
    );
    this.reporter.finishAction();
    return wallets;
  }
  ...
  private async distributeL1BaseToken(
    ...
    const ethIsBaseToken =
      baseTokenAddress == zksync.utils.ETH_ADDRESS_IN_CONTRACTS;
    ...
    if (!ethIsBaseToken) {
      ...
      const baseMintPromise = l1Erc20Contract
        ...
        .then((tx: any) => {
          this.reporter.debug(
            `Sent ERC20 mint transaction. Hash: ${tx.hash}, tx nonce ${tx.nonce}`,
          );
          return tx.wait();
        });
      this.reporter.debug(
        `Nonce changed by 1 for ERC20 mint, new nonce: ${nonce}`,
      );
      ...
      const baseDepositPromise = this.mainSyncWallet
        .deposit({
          ...
          overrides: {
            nonce:
              nonce + (ethIsBaseToken ? 0 : 1) + (baseIsTransferred ? 0 : 1), // if base is transferred, we don't need to approve override
            gasPrice,
          },
        })
        .then((tx) => {
          ...
          this.reporter.debug(
            `Sent ERC20 deposit transaction. Hash: ${tx.hash}, tx nonce: ${tx.nonce}`,
          );
          return tx.wait();
        });
      nonce =
        nonce + 1 + (ethIsBaseToken ? 0 : 1) + (baseIsTransferred ? 0 : 1);
      this.reporter.debug(
        `Nonce changed by ${
      ...
      this.reporter.debug(
        `Sent ${l1TxPromises.length} base token initial transactions on L1`,
      );
  ...
  private async distributeL1Tokens(
    ...
    if (l2ETHAmountToDeposit != 0n) {
      ...
      const depositHandle = this.mainSyncWallet
        ...
        .then((tx) => {
          ...
          this.reporter.debug(
            `Sent ETH deposit. Nonce ${tx.nonce}, amount: ${amount}, hash: ${tx.hash}`,
          );
          return tx.wait();
        });
    ...
    this.reporter.debug(
      `Sent ${l1TxPromises.length} initial transactions on L1`,
    );
    await Promise.all(l1TxPromises);
  ...
  private async waitForVmPlayground() {
    while (true) {
      ...
      if (lastProcessedBatch === undefined) {
        this.reporter.warn(
          "The node does not run VM playground; run to check old / new VM divergence",
        );
        break;
      }
      const lastNodeBatch = await this.l2Provider.getL1BatchNumber();
      this.reporter.debug(
        `VM playground progress: L1 batch #${lastProcessedBatch} / ${lastNodeBatch}`,
      );
  ...
  private async lastPlaygroundBatch() {
    ...
    const nodeHealth = (await (
      await fetch(`http://127.0.0.1:${healthcheckPort}/health`)
    ).json()) as NodeHealth;
    ...
    if (playgroundHealth.status !== "ready") {
      throw new Error(
        `Unexpected VM playground health status: ${playgroundHealth.status}`,
      );
    }
  ...
  private async checkVmDivergencesInApi() {
    ...
    if (vmMode !== "shadow") {
      this.reporter.warn(
        `API VM mode is '${vmMode}'; should be set to 'shadow' to check VM divergence`,
      );
      return;
    }
  ...
  private async collectFunds() {
    ...
    const l1Wallets = Object.values(this.wallets!).map(
      (pk) => new ethers.Wallet(pk, this.l1Provider),
    );
    ...
    const txPromises: ReceiptFuture[] = await claimEtherBack(
      wallets,
      this.mainEthersWallet.address,
      this.reporter,
    );
...
export async function sendTransfers(
  ...
  for (let index = 0; index < walletsPK.length; index++) {
    ...
    if (token == zksync.utils.ETH_ADDRESS) {
      ...
      reporter?.debug(
        `Sent ETH transfer tx: ${transactionResponse.hash}, nonce: ${transactionResponse.nonce}`,
      );
      ...
      const gasLimit = await erc20Contract.transfer.estimateGas(
        ethers.computeAddress(testWalletPK),
        value,
      );
