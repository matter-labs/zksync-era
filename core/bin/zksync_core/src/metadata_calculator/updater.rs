...
use zksync_types::{writes::InitialStorageWrite, L1BatchNumber, U256};
...
impl TreeUpdater {
    ...
    async fn process_multiple_batches(
        ...
        batches: Vec<L1BatchWithLogs>,
    ) {
        let start = Instant::now();

        let compute_latency = TreeUpdateStage::Compute.start();
        let total_logs: usize = batches.iter().map(|batch| batch.storage_logs.len()).sum();
        ...
        let (storage_logs, l1_batch_headers): (Vec<_>, Vec<_>) = batches
            .into_iter()
            .map(|batch| (batch.storage_logs, batch.header))
            .unzip();
        let mut previous_root_hash = self.tree.root_hash();
        let tree_metadata = self.tree.process_l1_batches(storage_logs).await;
        compute_latency.report();

        let mut updated_headers = Vec::with_capacity(l1_batch_headers.len());
        for (mut metadata, header) in tree_metadata.into_iter().zip(l1_batch_headers) {
            ...
            storage
                ...
                .await;
            // ^ Note that `save_blocks_metadata()` will not blindly overwrite changes if L1 batch
            ...
            updated_headers.push(l1_batch_with_metadata.header);
        }
    ...
    async fn step(
        ...
        if l1_batch_numbers.is_empty() {
            ...
            vlog::info!(
                "Loading L1 batches with numbers {l1_batch_numbers:?} to update Merkle tree"
            );
        }

        let mut new_l1_batches = vec![];
        for number in l1_batch_numbers {
            let logs = L1BatchWithLogs::new(&mut storage, L1BatchNumber(number)).await;
            new_l1_batches.extend(logs);
        }

        if let Some(last_l1_batch) = new_l1_batches.last() {
            load_changes_latency.report();
            *next_l1_batch_to_seal = last_l1_batch.header.number + 1;
            self.process_multiple_batches(&mut storage, &mut prover_storage, new_l1_batches)
                .await;
        }
    }
