...
export async function validateMigrations() {
    ...
    const files = filesRaw.trim().split('\n').sort().filter(filepath => !whitelistedMigrations.includes(filepath));
    const validateResults = await Promise.all(files.map((file) => validateMigration(file)));
    if (validateResults.includes(false)) {
        throw new Error('A new migration that is not backwards-compatible was detected! As such migrations can easily cause ');
    }
}
