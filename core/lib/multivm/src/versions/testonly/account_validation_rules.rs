...
pub(crate) fn test_account_validation_rules<VM: TestedVm + TestedVmForValidation>() {
    let (result, violated_rule) = test_rule::<VM>(u32::MAX, 0);
    assert!(!result.result.is_failed(), "{result:#?}");
    assert_matches!(violated_rule, None);

    let (result, violated_rule) = test_rule::<VM>(u32::MAX, 1);
    assert_matches!(
        &result.result,
    ...
    let (result, violated_rule) = test_rule::<VM>(u32::MAX, 2);
    assert_matches!(
        &result.result,
        ExecutionResult::Halt {
            reason: Halt::TracerCustom(_)
    ...
    let (result, violated_rule) = test_rule::<VM>(u32::MAX, 3);
    assert!(!result.result.is_failed(), "{result:#?}");
    assert_matches!(violated_rule, None);

    let (result, violated_rule) = test_rule::<VM>(u32::MAX, 4);
    assert_matches!(
        &result.result,
        ExecutionResult::Halt {
            reason: Halt::TracerCustom(_)
...
pub(crate) fn test_validation_out_of_gas_with_full_tracer<VM: TestedVmForValidation>() {
    let (result, violated_rule) = test_rule::<VM>(300_000, 4);
    assert_matches!(
        &result.result,
        ExecutionResult::Halt {
            reason: Halt::TracerCustom(_)
