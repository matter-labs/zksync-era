...
impl WsTest for SendRawTransactionSyncImmediateReceiptTest {
    async fn test(
        ...
        let mut storage = pool.connection().await?;
        store_l2_block(&mut storage, L2BlockNumber(1), &[mock_execute_transaction(create_l2_transaction(1, 2).into())])
            .await?;
        drop(storage);

        wait_for_notifier_l2_block(&mut pub_sub_events, SubscriptionType::Blocks, L2BlockNumber(1))
            .await;

        let receipt: api::TransactionReceipt = client
            .request("eth_sendRawTransactionSync", rpc_params![Bytes(tx_bytes), Option::<U256>::None])
            .await?;
...
impl WsTest for SendRawTransactionSyncDelayedReceiptTest {
    async fn test(
        ...
        let sync_task = tokio::spawn(async move {
            client_clone
                .request("eth_sendRawTransactionSync", rpc_params![Bytes(tx_bytes_clone), Some(U256::from(5000))])
                .await
        });
...
impl WsTest for SendRawTransactionSyncTimeoutTest {
    async fn test(
        ...
        if let ClientError::Call(e) = &err {
            assert_eq!(e.code(), 4);
            assert!(e.message().contains("timeout"), "Error message: {}", e.message());
        } else {
...
impl WsTest for SendRawTransactionSyncMultipleBlocksTest {
    async fn test(
        ...
        let mut storage = pool.connection().await?;
        store_l2_block(&mut storage, L2BlockNumber(1), &[])
            .await?;
        drop(storage);
        ...
        let mut storage = pool.connection().await?;
        store_l2_block(&mut storage, L2BlockNumber(2), &[mock_execute_transaction(create_l2_transaction(1, 2).into())])
            .await?;
        drop(storage);

        wait_for_notifier_l2_block(&mut pub_sub_events, SubscriptionType::Blocks, L2BlockNumber(2))
            .await;
...
impl WsTest for SendRawTransactionSyncDefaultTimeoutTest {
    async fn test(
        ...
        let sync_task = tokio::spawn(async move {
            client_clone
                .request("eth_sendRawTransactionSync", rpc_params![Bytes(tx_bytes_clone), Option::<U256>::None])
                .await
        });
