...
impl EthTxAggregator {
    ...
    pub(super) async fn save_eth_tx(
        ...
        let eth_tx_predicted_gas = match (op_type, is_gateway) {
            ...
            (AggregatedActionType::Commit, false) => match aggregated_op {
                AggregatedOperation::Commit(_, _, _, mode)
                    if *mode == L1BatchCommitmentMode::Validium =>
                {
                    Some(L1GasCriterion::total_validium_commit_gas_amount(
                        l1_batch_number_range.clone(),
                    ))
                }
                _ => None,
