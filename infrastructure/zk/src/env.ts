...
export function load_docker() {
    const in_docker: number = parseInt(process.env.IN_DOCKER || '0');
    if (!in_docker) {
        return;
    }
...
export async function load() {
    ...
    if (zksyncEnv == 'dev') {
        // If there no folder with toml files (or it's empty) we should delete
        // the old dev.env and regenerate toml files
        if (!fs.existsSync('etc/env/dev') || fs.readdirSync('etc/env/dev').length == 0) {
