...
impl ProofDataHandlerClient {
    ...
    pub async fn run(self, stop_receiver: watch::Receiver<bool>) -> anyhow::Result<()> {
        ...
        let proof_gen_data_submitter = ProofGenDataSubmitter::new(self.blob_store.clone(), self.pool.clone(), self.config.clone(), self.batch_commitment_mode).run(stop_receiver.clone());
        let proof_fetcher = ProofFetcher::new(self.blob_store, self.pool, self.config, self.batch_commitment_mode).run(stop_receiver);
