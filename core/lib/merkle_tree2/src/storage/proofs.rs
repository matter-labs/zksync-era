...
use crate::{
    hasher::{HashTree, HasherWithStats, HashingStats, MerklePath},
    storage::{
        Database, NewLeafData, PatchSet, SortedKeys, Storage, StorageMetrics, TraverseOutcome,
        TreeUpdater,
    },
    types::{
...
impl<'a, DB: Database + ?Sized> Storage<'a, DB> {
    pub fn extend_with_proofs(
        ...
        let (storage_parts, logs): (Vec<_>, Vec<_>) = storage_parts
            ...
            .enumerate()
            .map_init(
                || hasher.with_stats(&hashing_stats),
        ...
        let mut hasher = hasher.with_stats(&hashing_stats);
    ...
    fn finalize_with_proofs(
        ...
        let patch = self
            .updater
            .patch_set
            .finalize_without_hashing(self.leaf_count, hasher.as_ref());
