...
use super::metrics::{LoadChangesStage, ReportStage};
...
impl AsyncTree {
    ...
    pub async fn new(db_path: PathBuf, mode: MerkleTreeMode, block_cache_capacity: usize) -> Self {
    ...
    pub async fn process_l1_batches(
        &mut self,
        storage_logs: Vec<Vec<StorageLog>>,
    ) -> Vec<TreeMetadata> {
        let mut tree = mem::take(self);
        let (tree, metadata) = tokio::task::spawn_blocking(move || {
            tree.as_mut().reset(); // For compatibility with the old implementation
            let metadata = storage_logs
                .iter()
                .map(|logs| tree.as_mut().process_l1_batch(logs))
                .collect();
            (tree, metadata)
        })
        .await
...
mod tests {
    ...
    use zksync_types::{
        proofs::PrepareBasicCircuitsJob, system_contracts::get_system_smart_contracts, Address,
        L2ChainId, StorageKey, StorageLogKind,
    };
    ...
    async fn loaded_logs_equivalence_with_zero_no_op_logs(pool: ConnectionPool) {
        ...
        let mut tree = AsyncTree::new(temp_dir.path().to_owned(), MerkleTreeMode::Full, 0).await;
        for number in 0..3 {
