...
describe("Upgrade test", function () {
  ...
  before("Create test wallet", async () => {
    ...
    if (fileConfig.loadFromFile) {
      ...
      web3JsonRpc =
        process.env.ZKSYNC_WEB3_API_URL ||
        process.env.API_WEB3_JSON_RPC_HTTP_URL;
      contractsL2DefaultUpgradeAddr =
        process.env.CONTRACTS_L2_DEFAULT_UPGRADE_ADDR!;

      upgradeAddress = process.env.CONTRACTS_DEFAULT_UPGRADE_ADDR;
      if (!upgradeAddress) {
        throw new Error("CONTRACTS_DEFAULT_UPGRADE_ADDR not set");
      }
      contractsPriorityTxMaxGasLimit =
    ...
    if (fileConfig.loadFromFile) {
      ...
      adminGovWallet = new ethers.Wallet(
      ...
      let govWalletHD = ethers.HDNodeWallet.fromMnemonic(
        govMnemonic,
        "m/44'/60'/0'/0/1"
      );
      adminGovWallet = new ethers.Wallet(
        govWalletHD.privateKey,
        alice._providerL1()
      );
      ecosystemGovWallet = adminGovWallet;
    }
...
async function prepareUpgradeCalldata(
  ...
  const stmUpgradeCalldata = contracts.stateTransitonManager.encodeFunctionData(
  ...
  const stmUpgradeData = prepareGovernanceCalldata(
    stmAddress,
    stmUpgradeCalldata
  );
...
function prepareGovernanceCalldata(
  to: string,
  data: BytesLike
): UpgradeCalldata {
  const call = {
