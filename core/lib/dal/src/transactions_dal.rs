...
use zksync_types::{
    ...
    ProtocolVersionId, Transaction, H160, H256, PROTOCOL_UPGRADE_TX_TYPE, U256,
};
...
impl TransactionsDal<'_, '_> {
    pub async fn insert_transaction_l1(
        ...
        let contract_address = if tx.execute.contract_address.is_some() {
            tx.execute.contract_address.unwrap().as_bytes().to_vec()
        } else {
            vec![0; 32]
        };
        let tx_hash = tx.hash();
    ...
    async fn handle_executed_l2_transactions(
        ...
        for (index_in_block, tx_res) in transactions.iter().enumerate() {
            ...
            let contract_address = if transaction.execute.contract_address.is_some() {
                transaction
                    .execute
                    .contract_address
                    .unwrap()
            ...
            l2_contract_addresses.push(contract_address);
