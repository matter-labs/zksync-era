...
testFees("Test fees", function () {
  ...
  beforeAll(async () => {
    ...
    aliceErc20 = new ethers.Contract(
  ...
  test("Test gas price expected value", async () => {
    ...
    const conversionRatio: { numerator: bigint; denominator: bigint } = (
      feeParams.V2 as any
    )["conversion_ratio"];
    if (isETHBasedChain) {
    ...
    const expectedETHGasPrice =
      ...
      (feeConfig.compute_overhead_part *
        feeParams.V2.l1_gas_price *
        feeConfig.batch_overhead_l1_gas) /
        feeConfig.max_gas_per_batch;
    const expectedConvertedGasPrice =
      (expectedETHGasPrice * conversionRatio.numerator) /
  ...
  test("Test base token ratio fluctuations", async () => {
    ...
    const beginL1Nominator =
      await mainContract.baseTokenGasPriceMultiplierNominator();
    let changedL2 = false;
    let changedL1 = false;
    for (let i = 0; i < 20; i++) {
      await sleep(0.5);
  ...
  test("Test gas consumption under large L1 gas price", async () => {
    ...
    const systemContextGasPerPubdataByte =
      await systemContext.gasPerPubdataByte();
    expect(systemContextGasPerPubdataByte).toEqual(MAX_GAS_PER_PUBDATA);

    const dataHash = await l1Messenger.sendToL1.staticCall(largeData, {
      type: 0,
    });
  ...
  afterAll(async () => {
    ...
    __ZKSYNC_TEST_CONTEXT_OWNER__.setL2NodePid(
      mainNodeSpawner.mainNode!.proc.pid!,
    );
  });
});
...
async function appendResults(
  ...
  if (
    originalL1Receipts.length !== reports.length &&
    originalL1Receipts.length !== transactionRequests.length
  ) {
  ...
  for (let i = 0; i < originalL1Receipts.length; i++) {
    ...
    results.push(
      await updateReport(sender, receipt, request, newL1GasPrice, oldReport),
    );
  }
...
async function updateReport(
  ...
  const expectedL1Price = +ethers.formatEther(
    l1Receipt.gasUsed * newL1GasPrice,
  );
  // This is flaky without multiplying by 3.
  const estimatedL2GasPrice =
    ethers.getBigInt(await sender.provider.send("eth_gasPrice", [])) * 3n;
