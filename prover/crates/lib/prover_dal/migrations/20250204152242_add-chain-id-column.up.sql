...
ALTER TABLE leaf_aggregation_witness_jobs_fri DROP CONSTRAINT IF EXISTS leaf_aggregation_witness_jobs_fri_composite_index;
CREATE UNIQUE INDEX leaf_aggregation_witness_jobs_fri_composite_index ON leaf_aggregation_witness_jobs_fri (l1_batch_number, chain_id, circuit_id);

ALTER TABLE node_aggregation_witness_jobs_fri DROP CONSTRAINT IF EXISTS node_aggregation_witness_jobs_fri_composite_index;
CREATE UNIQUE INDEX node_aggregation_witness_jobs_fri_composite_index ON node_aggregation_witness_jobs_fri (l1_batch_number, chain_id, circuit_id, depth);

ALTER TABLE prover_jobs_fri DROP CONSTRAINT IF EXISTS prover_jobs_fri_composite_index;
CREATE UNIQUE INDEX prover_jobs_fri_composite_index ON prover_jobs_fri (l1_batch_number, chain_id, aggregation_round, circuit_id, depth, sequence_number);
