...
describe("Paymaster tests", () => {
  ...
  test("Should deploy a paymaster", async () => {
    ...
    await alice
      .transfer({
        ...
        amount: L2_DEFAULT_ETH_PER_ACCOUNT / 4n,
      })
      .then((tx) => tx.wait());
  });
  ...
  test("Should pay fee with paymaster", async () => {
    ...
    const paymasterParamsForEstimation =
      await getTestPaymasterParamsForFeeEstimation(
        erc20,
        alice.address,
        paymasterAddress,
      );
    ...
    await expect(txPromise).toBeAccepted([
      checkReceipt(
        (receipt) =>
          paidFeeWithPaymaster(
            receipt,
            CUSTOM_PAYMASTER_RATE_NUMERATOR,
            paymasterAddress,
          ),
  ...
  test("Should call postOp of the paymaster", async () => {
    ...
    const calledContextWithCounter =
      await paymaster.calledContext(afterCounter);
  ...
  test("Should pay fees with testnet paymaster", async () => {
    ...
    const testnetPaymaster =
      (await alice.provider.getTestnetPaymasterAddress())!;
    ...
    const paramsForFeeEstimation = zksync.utils.getPaymasterParams(
      testnetPaymaster,
      {
        type: "ApprovalBased",
    ...
    await expect(txPromise).toBeAccepted([
      checkReceipt(
        (receipt) =>
          paidFeeWithPaymaster(
            receipt,
            TESTNET_PAYMASTER_RATE_NUMERATOR,
            testnetPaymaster,
          ),
        "Fee was not paid (or paid incorrectly)",
      ),
...
function paidFeeWithPaymaster(
  ...
  const expectedErc20Fee = getTestPaymasterFeeInToken(
    fee.feeBeforeRefund,
    ratioNumerator,
  );
  ...
  if (!erc20FeeLog) {
    ...
    expect(null).fail(
      errorMessage(
        "Transaction did not have the ERC20 fee log (or the amount was incorrect)",
      ),
    );
...
function getTestPaymasterInnerInput(
...
function getTestPaymasterParams(
  ...
  const tokenAmount = getTestPaymasterFeeInToken(
    ethNeeded,
    CUSTOM_PAYMASTER_RATE_NUMERATOR,
  );
...
async function sendTxWithTestPaymasterParams(
  ...
  const paymasterParams = getTestPaymasterParams(
    paymasterAddress,
    token,
    ethNeeded,
    paymasterSignature,
