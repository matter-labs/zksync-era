...
fn test_default_aa_interaction() {
    ...
    let maximal_fee = tx.gas_limit() * vm.vm.batch_env.base_fee(VmVersion::VmBoojumIntegration);
    ...
    let expected_fee = maximal_fee
        // FIXME: once the new VM is integrated, latest should have its own VmVersion assigned.
        - U256::from(result.refunds.gas_refunded) * U256::from(vm.vm.batch_env.base_fee(VmVersion::VmBoojumIntegration));
