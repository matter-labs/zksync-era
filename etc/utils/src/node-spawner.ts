...
export class NodeSpawner {
  ...
  public async killAndSpawnMainNode(
    configOverrides: MainNodeOptions | null = null,
  ): Promise<void> {
  ...
  private async spawnMainNode(
    overrides: MainNodeOptions | null,
  ): Promise<Node<NodeType.MAIN>> {
    const env = this.env ?? process.env;
    const { fileConfig, pathToHome, options, logs } = this;

    const testMode =
      overrides?.newPubdataPrice != null || overrides?.newL1GasPrice != null;
    ...
    if (fileConfig.loadFromFile) {
      ...
      if (overrides != null) {
        if (overrides.newL1GasPrice) {
          config["eth"]["gas_adjuster"]["internal_enforced_l1_gas_price"] =
            overrides.newL1GasPrice;
        }

        if (overrides.newPubdataPrice) {
          config["eth"]["gas_adjuster"]["internal_enforced_pubdata_price"] =
            overrides.newPubdataPrice;
        }
        ...
        if (overrides.baseTokenPricePollingIntervalMs !== undefined) {
          const cacheUpdateInterval =
            overrides.baseTokenPricePollingIntervalMs / 2;
          ...
          config["base_token_adjuster"]["l1_tx_sending_sleep_ms"] =
            overrides.baseTokenPricePollingIntervalMs;
          config["base_token_adjuster"]["price_polling_interval_ms"] =
            overrides.baseTokenPricePollingIntervalMs;
          config["base_token_adjuster"]["price_cache_update_interval_ms"] =
            cacheUpdateInterval;
        }

        if (
          overrides.baseTokenAdjusterL1UpdateDeviationPercentage !== undefined
        ) {
      ...
      if (overrides != null) {
        ...
        if (overrides.newL1GasPrice) {
          ...
          env["ETH_SENDER_GAS_ADJUSTER_INTERNAL_ENFORCED_L1_GAS_PRICE"] =
            overrides.newL1GasPrice.toString();
        }
        ...
        if (overrides.baseTokenPricePollingIntervalMs !== undefined) {
          ...
          env["BASE_TOKEN_ADJUSTER_PRICE_POLLING_INTERVAL_MS"] =
            overrides.baseTokenPricePollingIntervalMs.toString();
          env["BASE_TOKEN_ADJUSTER_PRICE_CACHE_UPDATE_INTERVAL_MS"] =
            cacheUpdateInterval.toString();
        }
  ...
  private readFileConfig() {
    if (this.generalConfigPath == void 0)
      throw new Error(
        "Trying to set property in config while not in file mode",
      );
