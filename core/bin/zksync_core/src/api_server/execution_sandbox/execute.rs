...
use vm::{
    constants::ETH_CALL_GAS_LIMIT, HistoryDisabled, StorageInvocations, TxExecutionMode,
        ...
        let result = apply::apply_vm_in_sandbox(
            ...
            |vm, tx| {
                ...
                let custom_tracers: Vec<_> = custom_tracers
                    .into_iter()
                    .map(|tracer| tracer.into_boxed(HistoryDisabled))
                    .chain(vec![Box::new(storage_invocation_tracer) as _])
                    .collect();
