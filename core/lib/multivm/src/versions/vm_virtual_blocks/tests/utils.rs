...
use crate::vm_virtual_blocks::tests::tester::InMemoryStorageView;
...
use crate::vm_virtual_blocks::HistoryMode;
...
pub(crate) fn get_bootloader(test: &str) -> SystemContractCode {
    let bootloader_code = read_zbin_bytecode(format!(
        "etc/system-contracts/bootloader/tests/artifacts/{}.yul.zbin",
        test
    ));
