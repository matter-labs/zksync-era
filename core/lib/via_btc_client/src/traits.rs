...
pub trait BitcoinClient: Send + Sync {
    ...
    async fn broadcast_signed_transaction(&self, signed_transaction: &str) -> types::BitcoinClientResult<String>;
...
pub trait BitcoinSigner: Send + Sync {
    ...
    where
        Self: Sized;
    async fn sign_transaction(&self, unsigned_transaction: &str) -> types::BitcoinSignerResult<String>;
}
...
pub trait BitcoinInscriber: Send + Sync {
    ...
    where
        Self: Sized;
    async fn inscribe(&self, message_type: &str, data: &str) -> types::BitcoinInscriberResult<String>;
}
...
pub trait BitcoinInscriptionIndexer: Send + Sync {
    ...
    where
        Self: Sized;
    async fn get_inscription_messages(&self, starting_block: u128, ending_block: u128) -> types::BitcoinInscriptionIndexerResult<Vec<&str>>;
    async fn get_specific_block_inscription_messages(&self, block_height: u128) -> types::BitcoinInscriptionIndexerResult<Vec<&str>>;
}
...
pub trait BitcoinWithdrawalTransactionBuilder: Send + Sync {
    ...
    where
        Self: Sized;
    async fn build_withdrawal_transaction(&self, address: &str, amount: u128) -> types::BitcoinTransactionBuilderResult<String>;
}