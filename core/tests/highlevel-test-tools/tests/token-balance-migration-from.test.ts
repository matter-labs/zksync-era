...
(shouldSkip ? describe.skip : describe)('Token balance migration FROM GW tests', function () {
    ...
    it('Can migrate token balances to GW', async () => {
        ...
        // Migrate token balances to gateway -- will just migrate the base token
    ...
    it('Can deposit a token to the chain after migrating from gateway', async () => {
        tokens.L1NativeDepositedToL2AfterMigrationFromGW = await ERC20Handler.deployTokenOnL1(chainRichWallet);
        // Fresh deposit after the chain migrated to gateway marks the asset ID as effectively migrated
        await tokens.L1NativeDepositedToL2AfterMigrationFromGW.deposit(chainHandler);
        const assetIdA = await tokens.L1NativeDepositedToL2AfterMigrationFromGW.assetId(chainHandler);
        await expect(
    ...
    it('Can migrate token balances to L1', async () => {
        ...
        // This also tests repeated migrations, as `L1NativeDepositedToL2AfterMigrationFromGW` was already effectively migrated
    ...
    it('Correctly assigns chain token balances after migrating token balances to L1', async () => {
        ...
        for (const tokenName of Object.keys(tokens)) {
            ...
            const l1Expected = baseBalance;

            // Tokens deposited AFTER migrating from gateway won't have a GWAT migration number set
            const depositedAfterFromGW =
                tokenName === 'L1NativeNotDepositedToL2' || tokenName === 'L1NativeDepositedToL2AfterMigrationFromGW';
    ...
    it('Can deposit a token to the chain after migrating balances from gateway', async () => {
