...
pub fn encode_input_to_hex(input: &AirbenderVerifierInput) -> anyhow::Result<String> {
    let encoded =
        bincode::serialize(input).context("Failed to serialize verifier input with bincode")?;
    ...
    for word in words {
        use std::fmt::Write;
        write!(hex, "{:08x}", word).context("Failed to write hex word")?;
    }
...
pub fn encode_to_words(bytes: &[u8]) -> anyhow::Result<Vec<u32>> {
    anyhow::ensure!(
        bytes.len() <= u32::MAX as usize,
        "Encoded input is larger than u32::MAX bytes"
    );
...
pub fn decode_from_words(words: &[u32]) -> anyhow::Result<Vec<u8>> {
    anyhow::ensure!(!words.is_empty(), "No words provided");
