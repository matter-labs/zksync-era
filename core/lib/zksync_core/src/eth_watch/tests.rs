...
use zksync_types::{
    ...
    protocol::{ProtocolUpgradeTx, ProtocolUpgradeTxCommonData},
...
async fn get_all_db_txs(storage: &mut StorageProcessor<'_, Server>) -> Vec<Transaction> {
...
async fn setup_db(connection_pool: &ConnectionPool<Server>) {
    connection_pool
        .access_storage()
        .await
        .unwrap()
