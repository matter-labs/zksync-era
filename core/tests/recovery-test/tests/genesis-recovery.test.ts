...
describe("genesis recovery", () => {
    ...
    while (
      ...
      if (!treeFetcherSucceeded) {
        ...
        if (
          status === "ready" &&
          details !== undefined &&
          details.last_updated_l1_batch !== undefined
        ) {
          console.log("Received tree health details", details);
          treeFetcherSucceeded =
            details.last_updated_l1_batch >= CATCH_UP_BATCH_COUNT;
        }
      }
      ...
      if (!reorgDetectorSucceeded) {
        ...
        if (status === "ready" && details !== undefined) {
          ...
          if (details.last_correct_l1_batch !== undefined) {
            reorgDetectorSucceeded =
              details.last_correct_l1_batch >= CATCH_UP_BATCH_COUNT;
          }
        }
      }
      ...
      if (!consistencyCheckerSucceeded) {
        ...
        if (status === "ready" && details !== undefined) {
          ...
          if (
            ...
            consistencyCheckerSucceeded =
              details.last_checked_batch >= CATCH_UP_BATCH_COUNT;
          }
        }
      }
    }
  ...
  step("get EN batch number", async () => {
    ...
    expect(externalNodeBatchNumber).to.be.greaterThanOrEqual(
      CATCH_UP_BATCH_COUNT,
    );
  });
  ...
  step("wait for tree to catch up", async () => {
    const mainNodeBatchNumber = await mainNode.getL1BatchNumber();
    expect(mainNodeBatchNumber).to.be.greaterThanOrEqual(
      externalNodeBatchNumber + CATCH_UP_BATCH_COUNT,
    );
    const catchUpBatchNumber = Math.min(
      mainNodeBatchNumber,
      externalNodeBatchNumber + CATCH_UP_BATCH_COUNT,
    );
    ...
    while (
      ...
      if (!treeSucceeded) {
        ...
        if (
          status === "ready" &&
          details !== undefined &&
          details.next_l1_batch_number !== undefined
        ) {
          console.log("Received tree health details", details);
      ...
      if (!reorgDetectorSucceeded) {
        ...
        if (status === "ready" && details !== undefined) {
          ...
          if (details.last_correct_l1_batch !== undefined) {
            reorgDetectorSucceeded =
              details.last_correct_l1_batch >= catchUpBatchNumber;
          }
        }
      }
