...
describe("Smart contract behavior checks: zkos", () => {
  ...
  test("Should deploy contract with a constructor", async () => {
    const contract1 = await deployContract(alice, contracts.constructor, [
      2,
      3,
      false,
    ]);
    ...
    const contract2 = await deployContract(alice, contracts.constructor, [
      5,
      10,
      false,
    ]);
  ...
  // takes ~minute with zkos currently
  ...
  test("Should fail an infinite loop transaction", async () => {
    if (testMaster.isFastMode()) {
      ...
      console.log(
        `This test is disabled. If you see this line, please check if the issue is resolved`,
      );
      return;
    }
    ...
    await expect(
      infiniteLoop.infiniteLoop({ gasLimit: 1_000_000, gasPrice }),
    ).toBeReverted([]);
  });
  ...
  test("Should not allow invalid constructor calldata", async () => {
    ...
    await expect(
      deployContract(alice, contracts.counter, randomWrongArgs),
    ).toBeRejected("incorrect number of arguments to constructor");
  });
  ...
  test.skip("Should not allow invalid contract bytecode", async () => {
    ...
    const send = (tx: any) =>
      alice.provider.send("eth_sendRawTransaction", [
        zksync.utils.serializeEip712(tx),
      ]);
    const call = (tx: any) =>
      alice.provider.send("eth_call", [alice.provider.getRpcTransaction(tx)]);
    const estimateGas = (tx: any) =>
      alice.provider.send("eth_estimateGas", [
        alice.provider.getRpcTransaction(tx),
      ]);
    // Prepares an invalid serialized transaction with the bytecode of provided length.
    const invalidTx = (length: number) =>
      invalidBytecodeTestTransaction(alice.provider, [new Uint8Array(length)]);

    const txWithUnchunkableBytecode = await invalidTx(17);
    const unchunkableError = "Bytecode length is not divisible by 32";
    await expect(send(txWithUnchunkableBytecode)).toBeRejected(
      unchunkableError,
    );
    ...
    /*
         ...
         */
    await expect(
      call(txWithUnchunkableBytecode),
    ).toBeRejected(/*unchunkableError*/);
    await expect(
      estimateGas(txWithUnchunkableBytecode),
    ).toBeRejected(/*unchunkableError*/);

    const txWithBytecodeWithEvenChunks = await invalidTx(64);
    const evenChunksError = "Bytecode has even number of 32-byte words";
    await expect(send(txWithBytecodeWithEvenChunks)).toBeRejected(
      evenChunksError,
    );
    ...
    /*
         ...
         */
    await expect(
      call(txWithBytecodeWithEvenChunks),
    ).toBeRejected(/*evenChunksError*/);
    await expect(
      estimateGas(txWithBytecodeWithEvenChunks),
    ).toBeRejected(/*evenChunksError*/);
    ...
    await expect(send(txWithTooLongBytecode)).toBeRejected(
      tooLongBytecodeError,
    );
    /*
        {
          code: 3,
         ...
         */
    await expect(
      call(txWithTooLongBytecode),
    ).toBeRejected(/*tooLongBytecodeError*/);
    await expect(
      estimateGas(txWithTooLongBytecode),
    ).toBeRejected(/*tooLongBytecodeError*/);
  });
  ...
  test("Should check that eth_call works with custom block tags", async () => {
    ...
    await expect(
      counterContract.get.staticCall({ blockTag: "pending" }),
    ).resolves.toEqual(counterValue);
  ...
  test("Should correctly process msg.value inside constructor and in ethCall", async () => {
    ...
    const contextContract = await deployContract(
      alice,
      contracts.context,
      [],
      undefined,
  ...
  test.skip("Should return correct error during fee estimation", async () => {
    ...
    /*
         ...
         */
    await expect(
      errorContract.require_long.estimateGas(),
    ).toBeRevertedEstimateGas(/*'longlonglong'*/);
    await expect(
      errorContract.require_long(),
    ).toBeRevertedEthCall(/*'longlonglong'*/);
  ...
  test.skip("Should check block properties for tx execution", async () => {
    ...
    const deploymentBlock =
      await contextContract.deploymentTransaction()!.blockNumber!;
    ...
    const deploymentl1Batch =
      deploymentBlockInfo.l1BatchNumber ??
      (await alice.provider.getL1BatchNumber()) + 1;

    // Check that block gas limit is correct.
    const blockGasLimit = await contextContract.getBlockGasLimit({
      blockTag: "pending",
    });
    ...
    await expect(
      contextContract.checkBlockNumber(
        newL1Batch,
        newL1Batch + acceptedBlockLag,
      ),
    ).toBeAccepted([]);
    ...
    await expect(
      contextContract.checkBlockTimestamp(
        initialTimestamp,
        initialTimestamp + acceptedTimestampLag,
      ),
    ).toBeAccepted([]);
  });
