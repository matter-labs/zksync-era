...
The default configuration is `dev.env`, which is generated automatically from `dev.env.example` during `zk init` command
execution.
...
The server is configured using env files in `./etc/env` directory. After the first initialization, the file
...
Get the service_account.json file containing the GCP credentials from Kubernetes secret for the relevant environment(stage2/
...
## Running the setup key generator on a machine with GPU

- uncomment `"core/bin/setup_key_generator_and_server",` from root `Cargo.toml` file.
- ensure that the setup_2^26.key is in the current directory, the file can be downloaded from
...
## Running the setup key generator on a machine without GPU
...
```shell
# To process a fixed number of jobs
...
We don't expect this error to happen as we've updated to the latest version which fixes the problem.
...
don't expect this error to happen as we've updated to the latest version which fixes the problem.
