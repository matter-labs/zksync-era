...
fn test_get_used_contracts() {
    ...
    assert!(known_bytecodes_without_system_codes(&vm.vm).is_empty());
    ...
    assert_eq!(
        ...
        known_bytecodes_without_system_codes(&vm.vm)
            .keys()
            .cloned()
            .collect::<HashSet<U256>>()
    );
    ...
    for factory_dep in tx2.execute.factory_deps.unwrap() {
        ...
        assert!(known_bytecodes_without_system_codes(&vm.vm)
            .keys()
...
fn known_bytecodes_without_system_codes<S: WriteStorage, H: HistoryMode>(
    vm: &Vm<S, H>,
) -> HashMap<U256, Vec<U256>> {
    let mut known_bytecodes_without_system_codes = vm
        .state
    ...
    known_bytecodes_without_system_codes
