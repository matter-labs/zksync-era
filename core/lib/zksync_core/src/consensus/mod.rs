...
use storage::{Store,ConnectionPool};
...
async fn run_main_node(ctx: &ctx::Ctx, cfg: ConsensusConfig, secrets: ConsensusSecrets, pool: ConnectionPool, chain_id: validator::ChainId) -> anyhow::Result<()> {
    let validator_key = config::validator_key(&secrets).context("validator_key")?.context("missing validator_key")?;
    scope::run!(&ctx, |ctx, s| async {
        pool
            .connection(ctx).await.wrap("connection()")?
            .try_init_genesis(ctx, chain_id, &validator_key.public()).await.wrap("try_init_genesis()")?;
