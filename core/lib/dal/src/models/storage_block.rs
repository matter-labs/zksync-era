...
/// Returns block_number SQL statement and the next argument index that can be used
pub fn web3_block_number_to_sql(block_number: api::BlockNumber, arg_index: u8) -> (String, u8) {
    match block_number {
        api::BlockNumber::Earliest => ("(SELECT 0::bigint as number)".to_string(), arg_index),
        ...
        api::BlockNumber::Number(_) => {
            (format!("(SELECT ${} as number)", arg_index), arg_index + 1)
        }
        ...
        api::BlockNumber::Finalized => (
            "
                (SELECT COALESCE(
                    (
                        SELECT miniblocks.number FROM miniblocks
                        JOIN l1_batches ON miniblocks.l1_batch_number = l1_batches.number
                        JOIN eth_txs ON l1_batches.eth_execute_tx_id = eth_txs.id
                        WHERE eth_txs.confirmed_eth_tx_history_id IS NOT NULL
                        ORDER BY miniblocks.number DESC
                        LIMIT 1
                    ),
                    0
                ) as number)
...
pub fn web3_block_where_sql(block_id: BlockId, arg_index: u8) -> String {
    match block_id {
        ...
        BlockId::Number(number) => {
            let block_sql = web3_block_number_to_sql(number, arg_index).0;
...
pub fn bind_block_where_sql_params(
    ...
    match block_id {
        ...
        BlockId::Number(api::BlockNumber::Number(number)) => query.bind(number.as_u64() as i64),
...
impl From<StorageBlockDetails> for BlockDetails {
    fn from(storage_block_details: StorageBlockDetails) -> Self {
        let status = if storage_block_details.number == 0
            || storage_block_details.execute_tx_hash.is_some()
        {
            BlockStatus::Verified
        } else {
            BlockStatus::Sealed
