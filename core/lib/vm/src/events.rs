...
use zksync_utils::{be_chunks_to_words, h256_to_account_address, u256_to_h256};
...
impl SolidityLikeEvent {
    pub fn into_vm_event(self, block_number: L1BatchNumber) -> VmEvent {
        VmEvent {
            location: (block_number, self.tx_number_in_block as u32),
            address: self.address,
            indexed_topics: be_chunks_to_words(self.topics)
                .into_iter()
                .map(u256_to_h256)
                .collect(),
            value: self.data,
