...
describe('Tests for the WETH bridge/token behavior', () => {
    ...
    test('Should transfer WETH', async () => {
        ...
        const balanceChange = await shouldChangeTokenBalances(aliceL2WrappedBaseToken.address, [
        ...
        await expect(aliceL2WrappedBaseToken.transfer(bob.address, value)).toBeAccepted([balanceChange, feeCheck]);
    });

    test('Can unwrap WETH on L2', async () => {
        const value = BigNumber.from(200);

        const tokenBalanceChange = await shouldChangeTokenBalances(aliceL2WrappedBaseToken.address, [
            { wallet: alice, change: -value }
        ]);
        const ethBalanceChange = await shouldChangeETHBalances([{ wallet: alice, change: value }]);
        await expect(aliceL2WrappedBaseToken.withdraw(value)).toBeAccepted([tokenBalanceChange, ethBalanceChange]);
    });

    test('Approve and transferFrom should work', async () => {
        const approveAmount = 42;
        const bobErc20 = aliceL2WrappedBaseToken.connect(bob);
        ...
        await expect(aliceL2WrappedBaseToken.approve(bob.address, approveAmount)).toBeAccepted();
        await expect(aliceL2WrappedBaseToken.allowance(alice.address, bob.address)).resolves.bnToBeEq(approveAmount);
