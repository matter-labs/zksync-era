...
describe("snapshot recovery", () => {
  ...
  const disableTreeDuringPruning =
    process.env.DISABLE_TREE_DURING_PRUNING === "true";
  console.log(
    `Tree is ${disableTreeDuringPruning ? "disabled" : "enabled"} during pruning`,
  );
  ...
  async function getSnapshot(snapshotL1Batch: number) {
    const output = await mainNode.send("snapshots_getSnapshot", [
      snapshotL1Batch,
    ]);
  ...
  step("deploy EVM bytecode if allowed", async () => {
    ...
    const contractDeployerAbi = readContract(
      systemContractsPath,
      "ContractDeployer",
    ).abi;
    ...
    const allowedBytecodeTypes =
      await contractDeployer.allowedBytecodeTypesToDeploy();
    ...
    if (allowedBytecodeTypes === 1n) {
      ...
      const erc20 = await (
        await erc20Factory.deploy("test", "TEST")
      ).waitForDeployment();
  ...
  step("validate snapshot", async () => {
    ...
    const SnapshotFactoryDependencies = root.lookupType(
      "zksync.types.SnapshotFactoryDependencies",
    );
    const SnapshotStorageLogsChunk = root.lookupType(
      "zksync.types.SnapshotStorageLogsChunk",
    );

    const factoryDepsPath = path.join(
      homeDir,
      snapshotMetadata.factoryDepsFilepath,
    );
    ...
    for (const chunkMetadata of snapshotMetadata.storageLogsChunks) {
      ...
      const output = SnapshotStorageLogsChunk.decode(
        await decompressGzip(chunkPath),
      ) as any as StorageLogChunk;
      ...
      for (const storageLog of output.storageLogs) {
        ...
        if (snapshotMetadata.version === 0) {
          const snapshotAccountAddress =
            "0x" + storageLog.accountAddress.toString("hex");
  ...
  step("initialize external node", async () => {
    ...
    while (
      !recoveryFinished ||
      !consistencyCheckerSucceeded ||
      !reorgDetectorSucceeded
    ) {
      await sleep(1000);
      ...
      if (!recoveryFinished) {
        ...
        if (details !== undefined) {
          ...
          expect(details.snapshot_l1_batch).to.equal(
            snapshotMetadata.l1BatchNumber,
          );
          expect(details.snapshot_l2_block).to.equal(
            snapshotMetadata.miniblockNumber,
          );
      ...
      if (!consistencyCheckerSucceeded) {
        ...
        if (status === "ready" && details !== undefined) {
          ...
          if (
            ...
            expect(details.first_checked_batch).to.equal(
              snapshotMetadata.l1BatchNumber + 1,
            );
            expect(details.last_checked_batch).to.be.greaterThan(
              snapshotMetadata.l1BatchNumber,
            );
      ...
      if (!reorgDetectorSucceeded) {
        ...
        if (status === "ready" && details !== undefined) {
          ...
          if (
            details.last_correct_l1_batch !== undefined &&
            details.last_correct_l2_block !== undefined
          ) {
            expect(details.last_correct_l1_batch).to.be.greaterThan(
              snapshotMetadata.l1BatchNumber,
            );
            expect(details.last_correct_l2_block).to.be.greaterThan(
              snapshotMetadata.miniblockNumber,
            );
  ...
  step("check tokens on EN", async () => {
    ...
    const mainNodeTokens = await getAllTokens(
      mainNode,
      snapshotMetadata.miniblockNumber,
    );
    ...
    const compareFn = (x: TokenInfo, y: TokenInfo) => {
      expect(x.l2_address, "Multiple tokens with same L2 address").to.not.equal(
        y.l2_address,
      );
  ...
  step(`wait for pruning ${PRUNED_BATCH_COUNT} L1 batches`, async () => {
    const expectedPrunedBatchNumber =
      snapshotMetadata.l1BatchNumber + PRUNED_BATCH_COUNT;
    console.log(
      `Waiting for L1 batch #${expectedPrunedBatchNumber} to be pruned`,
    );
    ...
    while (!isDbPruned || !isTreePruned) {
      ...
      isDbPruned =
        dbPrunerHealth.details!.last_hard_pruned_l1_batch! >=
        expectedPrunedBatchNumber;
      ...
      if (disableTreeDuringPruning) {
        ...
        isTreePruned = minTreeL1BatchNumber
          ? minTreeL1BatchNumber - 1 >= expectedPrunedBatchNumber
          : false;
      }
    }
  });
