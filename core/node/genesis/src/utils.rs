...
/// Returns batch output based on `entries` generated by the genesis L1 batch. This does not
/// create a persistent tree.
pub fn process_genesis_batch_in_tree(entries: &[TreeEntry]) -> BatchOutput {
    tracing::info!(
        "Creating Merkle tree for genesis batch with {instr_count} writes",
        instr_count = entries.len()
    );

    // `unwrap()`s are safe: in-memory trees never raise I/O errors
    let mut in_memory_tree = MerkleTree::new(PatchSet::default()).unwrap();
    let output = in_memory_tree.extend(entries).unwrap();
