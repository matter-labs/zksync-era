...
use zksync_types::{
    block::L1BatchHeader,
    commitment::{L1BatchCommitment, L1BatchMetadata, L1BatchWithMetadata},
};
...
impl<'a> MetadataCalculatorConfig<'a> {
    pub(crate) fn for_main_node(
        ...
        Self {
            db_path: &db_config.merkle_tree_path,
            mode,
            delay_interval: operation_config.delay_interval(),
            max_l1_batches_per_iter: db_config.max_l1_batches_per_tree_iter,
            block_cache_capacity: db_config.merkle_tree_block_cache_size(),
        }
    }
}
...
impl MetadataCalculator {
    ...
    async fn reestimate_l1_batch_commit_gas(
        ...
        let l1_batch_with_metadata =
            L1BatchWithMetadata::new(header, metadata, unsorted_factory_deps);
        let commit_gas_cost = commit_gas_count_for_l1_batch(&l1_batch_with_metadata);
        storage
            .blocks_dal()
            .update_predicted_l1_batch_commit_gas(
                l1_batch_with_metadata.header.number,
                commit_gas_cost,
            )
            .await;
