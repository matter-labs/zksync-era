...
fn create_backup(config: &DBConfig) -> Result<(), Error> {
    let mut engine = BackupEngine::open(
        &BackupEngineOptions::new(&config.merkle_tree_backup_path)?,
        &Env::new()?,
    )?;
    let db_dir = &config.merkle_tree_path;
...
mod tests {
    ...
    fn backup_restore_workflow() {
        ...
        let db_config = DBConfig {
            merkle_tree_path: temp_dir.path().to_str().unwrap().to_string(),
            merkle_tree_backup_path: backup_dir.path().to_str().unwrap().to_string(),
            ..DBConfig::from_env()
        };
        let db_dir = &db_config.merkle_tree_path;
