...
describe("Tests for the custom account behavior", () => {
  ...
  beforeAll(() => {
    ...
    timestampAsserterAddress =
      testMaster.environment().timestampAsserterAddress;
  ...
  test("Should deploy custom account", async () => {
    ...
    const contractAccountInfo = await alice.provider.getContractAccountInfo(
      await customAccount.getAddress(),
    );

    // Checking that the version of the account abstraction is correct
    expect(contractAccountInfo.supportedAAVersion).toEqual(
      zksync.types.AccountAbstractionVersion.Version1,
    );

    // Checking that the nonce ordering is correct
    expect(contractAccountInfo.nonceOrdering).toEqual(
      zksync.types.AccountNonceOrdering.Sequential,
    );
  ...
  test("Should execute contract by custom account", async () => {
    const tx = await erc20.transfer.populateTransaction(
      alice.address,
      TRANSFER_AMOUNT,
    );
  ...
  test("Should fail transaction validation due to timestamp assertion in the validation tracer - close to the range end", async () => {
    ...
    const minTimeTillEnd =
      testMaster.environment().timestampAsserterMinTimeTillEndSec;
  ...
  test("Should fail to estimate fee due to block.timestamp assertion in the smart contract", async () => {
    ...
    try {
      ...
      const functionSelectorMatch = err.message.match(
        /function_selector\s=\s(0x[0-9a-fA-F]{8})/,
      );
  ...
  test("Should not allow violating validation rules", async () => {
    ...
    let tx = await erc20.transfer.populateTransaction(
      alice.address,
      TRANSFER_AMOUNT,
    );
    await expect(
  ...
  test("Should not execute from non-account", async () => {
    ...
    // Fund the account.
    await alice
      .transfer({ to: nonAccountAddress, amount: ETH_PER_CUSTOM_ACCOUNT })
      .then((tx) => tx.wait());
    await alice
      .transfer({
  ...
  test("API should reject validation that takes too many computational ergs", async () => {
    ...
    const validationGasLimit =
      testMaster.environment().validationComputationalGasLimit;
    await badCustomAccount
      .setGasToSpent(validationGasLimit)
      .then((tx: any) => tx.wait());
  ...
  test("State keeper should reject validation that takes too many computational ergs", async () => {
    ...
    const transfer = await erc20.transfer.populateTransaction(
      alice.address,
      TRANSFER_AMOUNT,
    );
    const nonce = await alice.provider.getTransactionCount(
      badCustomAccountAddress,
    );
    ...
    const validationGasLimit =
      testMaster.environment().validationComputationalGasLimit;
    const tx =
      await badCustomAccount.setGasToSpent.populateTransaction(
        validationGasLimit,
      );
    await expect(
      sendCustomAccountTransaction(
...
async function sendCustomAccountTransaction(
  ...
  const gasLimit = estimateGas
    ? await browserProvider.estimateGas({ ...tx, from: accountAddress })
    : BigInt(100_000); // Enough gas to invoke AA contract
  ...
  tx.value = 0n;
  tx.nonce =
    nonce ?? (await browserProvider.getTransactionCount(accountAddress));
  tx.type = 113;
  ...
  tx.customData = {
    ...tx.customData,
    customSignature:
      customSignature ?? ethers.concat([signedTxHash, accountAddress]),
  };
