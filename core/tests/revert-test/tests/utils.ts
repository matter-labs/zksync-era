...
export async function executeRevert(
  ...
  assert(
    blocksCommitted === values.lastExecutedL1BatchNumber,
    "Revert on contract was unsuccessful",
  );
}
...
export class Node<TYPE extends NodeType> {
  ...
  public async createBatchWithDeposit(to: string, amount: bigint) {
    const initialL1BatchNumber =
      await this.tester.web3Provider.getL1BatchNumber();
    console.log(`Initial L1 batch: ${initialL1BatchNumber}`);

    const depositHandle = await this.tester.syncWallet.deposit({
      token: this.tester.isETHBasedChain
        ? zksync.utils.LEGACY_ETH_ADDRESS
        : this.tester.baseTokenAddress,
      amount,
      to,
...
export class NodeSpawner {
  ...
  public async spawnMainNode(
    ...
    replaceL1BatchMinAgeBeforeExecuteSeconds(
      pathToHome,
      chainName,
      enableExecute ? 0 : 10000,
    );

    let components =
      "api,tree,eth,state_keeper,commitment_generator,da_dispatcher,vm_runner_protective_reads";
...
async function waitForNodeToStart(
  tester: Tester,
  ...
  while (true) {
    try {
      ...
      console.log(
        `Initialized node API on ${l2Url}; latest block: ${blockNumber}`,
      );
      break;
    } catch (err) {
...
export async function waitToExecuteBatch(
  mainContract: IZkSyncHyperchain,
  latestBatch: number,
) {
  let tryCount = 0;
  ...
  while (
    (lastExecutedBatch = await mainContract.getTotalBatchesExecuted()) ===
...
export async function waitToCommitBatchesWithoutExecution(
  mainContract: IZkSyncHyperchain,
): Promise<bigint> {
  ...
  console.log(
    `Batches committed: ${batchesCommitted}, executed: ${batchesExecuted}`,
  );

  let tryCount = 0;
  while (
    (batchesExecuted === 0n || batchesCommitted === batchesExecuted) &&
    tryCount < 100
  ) {
    await utils.sleep(1);
  ...
  expect(
    batchesCommitted > batchesExecuted,
    "There is no committed but not executed batch",
  ).to.be.true;
...
export async function executeDepositAfterRevert(
  ...
  const depositHandle = await tester.syncWallet.deposit({
    token: tester.isETHBasedChain
      ? zksync.utils.LEGACY_ETH_ADDRESS
      : tester.baseTokenAddress,
    amount,
    to: wallet.address,
    approveBaseERC20: true,
    approveERC20: true,
  ...
  let l1TxResponse = await wallet
    ._providerL1()
    .getTransaction(depositHandle.hash);
  while (!l1TxResponse) {
    console.log(
      `Deposit ${depositHandle.hash} is not visible to the L1 network; sleeping`,
    );
    await utils.sleep(1);
    l1TxResponse = await wallet
      ._providerL1()
      .getTransaction(depositHandle.hash);
  }
  ...
  const l2Tx = await wallet
    ._providerL2()
    .getL2TransactionFromPriorityOp(l1TxResponse);
  let receipt = null;
  while (receipt === null) {
    console.log(
      `L2 deposit transaction ${l2Tx.hash} is not confirmed; sleeping`,
    );
    await utils.sleep(1);
...
export async function checkRandomTransfer(
  ...
  while (txReceipt === null) {
    ...
    await utils.sleep(1);
    txReceipt = await sender.provider.getTransactionReceipt(
      transferHandle.hash,
    );
  }
  ...
  assert(
    senderBalance + spentAmount >= senderBalanceBefore,
