...
impl<'a> From<CommitBatchInfo<'a>> for Token {
    fn from(self_: CommitBatchInfo<'a>) -> Token {
        if self_.0.header.protocol_version.unwrap().is_pre_boojum() {
            Token::Tuple(vec![
                ...
                Token::FixedBytes(self_.0.metadata.merkle_root_hash.as_bytes().to_vec()),
                ...
                Token::Bytes(self_.0.metadata.initial_writes_compressed.clone()),
                Token::Bytes(self_.0.metadata.repeated_writes_compressed.clone()),
                Token::Bytes(self_.0.metadata.l2_l1_messages_compressed.clone()),
                Token::Array(
                    self_
                        .0
                        .header
            ...
            Token::Tuple(vec![
                ...
                Token::Bytes(
                    self_
                        ...
                        .pubdata_input
                        .clone()
                        .unwrap_or(self_.0.construct_pubdata()),
                ),
            ])
        }
    }
