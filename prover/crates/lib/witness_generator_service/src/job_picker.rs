...
use crate::{
    ...
    rounds::{JobManager, JobMetadata, VerificationKeyManager},
};
    ...
    R: JobManager + ArtifactsManager,
{
    ...
    async fn pick_job(&mut self) -> anyhow::Result<Option<(R::Job, R::Metadata)>> {
        let start_time = Instant::now();
        tracing::info!("Starting picking witness generator {:?} job", R::ROUND);

        if let Some(job_metadata) = R::get_metadata(self.pool.clone(), self.protocol_version)
            .await
