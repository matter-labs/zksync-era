...
pub struct PrecalculatedMerklePathsProvider {
    ...
    root_hash: [u8; 32],
    // The list of expected leaves to be interacted with.
    pending_leaves: VecDeque<StorageLogMetadata>,
...
impl PrecalculatedMerklePathsProvider {
    pub fn new(input: PrepareBasicCircuitsJob, root_hash: [u8; 32]) -> Self {
        ...
        vlog::debug!(
            "Initializing PrecalculatedMerklePathsProvider. Initial root_hash: {:?}, \
             initial next_enumeration_index: {:?}",
            root_hash,
    ...
    for PrecalculatedMerklePathsProvider
{
    ...
    fn get_leaf(&mut self, index: &[u8; 32]) -> LeafQuery<256, 32, 32, 32, ZkSyncStorageLeaf> {
        ...
        let next = self.pending_leaves.front().unwrap_or_else(|| {
            panic!(
    ...
    fn insert_leaf(
        ...
        let next = self.pending_leaves.pop_front().unwrap();
        self.root_hash = next.root_hash;
    ...
    fn filter_renumerate<'a>(
        ...
        let mut write_pending_leaves = self.pending_leaves.iter().filter(|log| log.is_write);
        let write_pending_leaves = &mut write_pending_leaves;
        for ((pending_leaf, idx), mut leaf) in
            write_pending_leaves.zip(&mut indexes).zip(&mut leafs)
        {
        ...
        let length = first_writes.len() + updates.len();
        assert!(
            write_pending_leaves.next().is_none(),
            "pending leaves: len({}) must be of same length as leafs and indexes: len({})",
            write_pending_leaves.count() + 1 + length,
            // ^ 1 is added because of `next()` getting called in the assertion condition
            length
        );
        assert!(
    ...
    fn verify_inclusion(
        ...
        for level in 0..256 {
            let (lhs, rhs) = if is_right_side_node(&query.index, level) {
                (&query.merkle_path[level], &current_hash)
            } else {
                (&current_hash, &query.merkle_path[level])
            };
            current_hash = Blake2s256::node_hash(level, lhs, rhs);
        }
        *root == current_hash
    }
}

fn is_right_side_node(index: &[u8], depth: usize) -> bool {
    debug_assert!(depth < index.len() * 8);
