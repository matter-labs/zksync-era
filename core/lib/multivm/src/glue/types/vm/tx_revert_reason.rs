...
impl GlueFrom<vm_m5::TxRevertReason> for vm_latest::TxRevertReason {
    fn glue_from(value: vm_m5::TxRevertReason) -> Self {
        match value {
            ...
            vm_m5::TxRevertReason::ValidationFailed(err) => {
                Self::Halt(vm_latest::Halt::ValidationFailed(err.glue_into()))
            }
            vm_m5::TxRevertReason::PaymasterValidationFailed(err) => {
                Self::Halt(vm_latest::Halt::PaymasterValidationFailed(err.glue_into()))
            }
            vm_m5::TxRevertReason::PrePaymasterPreparationFailed(err) => Self::Halt(
                vm_latest::Halt::PrePaymasterPreparationFailed(err.glue_into()),
            ),
            vm_m5::TxRevertReason::PayForTxFailed(err) => {
                Self::Halt(vm_latest::Halt::PayForTxFailed(err.glue_into()))
            }
            vm_m5::TxRevertReason::FailedToMarkFactoryDependencies(err) => Self::Halt(
                vm_latest::Halt::FailedToMarkFactoryDependencies(err.glue_into()),
            ),
            vm_m5::TxRevertReason::FailedToChargeFee(err) => {
                Self::Halt(vm_latest::Halt::FailedToChargeFee(err.glue_into()))
            }
            vm_m5::TxRevertReason::FromIsNotAnAccount => {
                Self::Halt(vm_latest::Halt::FromIsNotAnAccount)
            }
            vm_m5::TxRevertReason::InnerTxError => Self::Halt(vm_latest::Halt::InnerTxError),
            vm_m5::TxRevertReason::Unknown(err) => {
                Self::Halt(vm_latest::Halt::Unknown(err.glue_into()))
            }
            vm_m5::TxRevertReason::UnexpectedVMBehavior(err) => {
                Self::Halt(vm_latest::Halt::UnexpectedVMBehavior(err))
            }
            vm_m5::TxRevertReason::BootloaderOutOfGas => {
                Self::Halt(vm_latest::Halt::BootloaderOutOfGas)
            }
            vm_m5::TxRevertReason::TooBigGasLimit => Self::Halt(vm_latest::Halt::TooBigGasLimit),
            vm_m5::TxRevertReason::NotEnoughGasProvided => {
                Self::Halt(vm_latest::Halt::NotEnoughGasProvided)
            }
        }
    }
}

impl GlueFrom<vm_m6::TxRevertReason> for vm_latest::TxRevertReason {
    fn glue_from(value: vm_m6::TxRevertReason) -> Self {
        match value {
            ...
            vm_m6::TxRevertReason::InnerTxError => Self::Halt(vm_latest::Halt::InnerTxError),
            ...
            vm_m6::TxRevertReason::TooBigGasLimit => Self::Halt(vm_latest::Halt::TooBigGasLimit),
            vm_m6::TxRevertReason::NotEnoughGasProvided => {
                Self::Halt(vm_latest::Halt::NotEnoughGasProvided)
            }
            vm_m6::TxRevertReason::MissingInvocationLimitReached => {
                Self::Halt(vm_latest::Halt::MissingInvocationLimitReached)
            }
        }
    }
}

impl GlueFrom<vm_1_3_2::TxRevertReason> for vm_latest::TxRevertReason {
    fn glue_from(value: vm_1_3_2::TxRevertReason) -> Self {
        match value {
            ...
            vm_1_3_2::TxRevertReason::PaymasterValidationFailed(err) => {
                Self::Halt(vm_latest::Halt::PaymasterValidationFailed(err.glue_into()))
            }
            ...
            vm_1_3_2::TxRevertReason::InnerTxError => Self::Halt(vm_latest::Halt::InnerTxError),
            ...
            vm_1_3_2::TxRevertReason::TooBigGasLimit => Self::Halt(vm_latest::Halt::TooBigGasLimit),
