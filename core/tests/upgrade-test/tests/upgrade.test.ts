...
const L2_FORCE_DEPLOY_UPGRADER_ABI = new ethers.utils.Interface(
    require(`${process.env.ZKSYNC_HOME}/contracts/zksync/artifacts-zk/contracts-preprocessed/ForceDeployUpgrader.sol/ForceDeployUpgrader.json`).abi
);
const COMPLEX_UPGRADER_ABI = new ethers.utils.Interface(
    require(`${process.env.ZKSYNC_HOME}/etc/system-contracts/artifacts-zk/contracts-preprocessed/ComplexUpgrader.sol/ComplexUpgrader.json`).abi
);
...
describe('Upgrade test', function () {
    ...
    step('Send l1 tx for saving new bootloader', async () => {
        const path = `${process.env.ZKSYNC_HOME}/etc/system-contracts/bootloader/build/artifacts/playground_batch.yul.zbin`;
