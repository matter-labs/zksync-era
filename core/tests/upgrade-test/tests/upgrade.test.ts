...
describe('Upgrade test', function () {
    ...
    step('Run server and execute some transactions', async () => {
        ...
        // TODO (PLA-815): Remove `IGNORE_1_4_2_UPGRADE=true`
        utils.background(
            'cd $ZKSYNC_HOME && IGNORE_1_4_2_UPGRADE=true cargo run --bin zksync_server --release -- --components=api,tree,eth,state_keeper,commitment_generator',
            [null, logs, logs]
        );
    ...
    step('Execute transactions after simple restart', async () => {
        ...
        // TODO (PLA-815): Remove `IGNORE_1_4_2_UPGRADE=true`
        utils.background(
            'cd $ZKSYNC_HOME && IGNORE_1_4_2_UPGRADE=true cargo run --bin zksync_server --release -- --components=api,tree,eth,state_keeper,commitment_generator &> upgrade.log',
            [null, logs, logs]
        );
        await utils.sleep(10);
