...
impl<S: Send + Sync> FromRequest<S> for ExternalProof {
    ...
    async fn from_request(req: Request, state: &S) -> Result<Self, Self::Rejection> {
        ...
        let proof: L1BatchProofForL1 = match zksync_object_store::bincode::deserialize::<
            PlonkL1BatchProofForL1,
        >(&serialized_proof)
        {
            Ok(proof) => Ok(proof.into()),
            Err(_) => zksync_object_store::bincode::deserialize::<FflonkL1BatchProofForL1>(
                &serialized_proof,
            )
            .map(Into::into),
        }?;
