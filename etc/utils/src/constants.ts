...
export const INTEROP_BUNDLE_ABI =
    'tuple(bytes1 version, uint256 destinationChainId, uint256 destinationBaseTokenAssetId, bytes32 interopBundleSalt, tuple(bytes1 version, bool shadowAccount, address to, address from, uint256 value, bytes data)[] calls, (address executionAddress, address unbundlerAddress) bundleAttributes)';
...
export const ArtifactL2InteropRootStorage = readContract(`${ARTIFACTS_PATH}`, 'L2InteropRootStorage');
...
export const ArtifactMintableERC20 = readContract(`${ARTIFACTS_PATH}`, 'TestnetERC20Token');
