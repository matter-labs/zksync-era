...
function readContract(
  name: string,
  kind: ContractKind = ContractKind.TEST,
): ContractData {
  let fileName = name;
  ...
  return JSON.parse(
    fs.readFileSync(`${basePath}/${fileName}.sol/${contractName}.json`, {
      encoding: "utf-8",
    }),
  );
}

async function getAllowedBytecodeTypes(
  provider: ethers.Provider,
): Promise<bigint> {
  const contractDeployerData = readContract(
    "ContractDeployer",
    ContractKind.SYSTEM,
  );
...
function describeEvm(providerKind: ProviderKind) {
  ...
  beforeAll(async () => {
    ...
    counterContractData = readContract("Counter:Counterr");
    counterFactory = new ethers.ContractFactory(
      counterContractData.abi,
      counterContractData.bytecode,
      alice,
    );
  });
  ...
  testEvm("Should deploy counter wallet", async () => {
    ...
    const counter = await (
      await counterFactory.deploy(false)
    ).waitForDeployment();
    ...
    const expectedBytecode = extractBytecode(
      counterContractData.deployedBytecode!!,
    );
    ...
    const incrementTx = await counter
      .getFunction("increment")
      .populateTransaction(42n);
  ...
  testEvm("Should estimate gas for EVM transactions", async () => {
    ...
    const incrementTx = await counterContract
      .getFunction("increment")
      .populateTransaction(42n);
  ...
  testEvm("Should propagate constructor revert", async () => {
    await expect(counterFactory.deploy(true)).rejects.toThrow(
      "requested revert",
    );
  });
  ...
  testEvm("Should define contractAddress for failed deployments", async () => {
    ...
    const deployTx = await counterFactory.getDeployTransaction(true, {
      gasLimit: 10_000_000,
    });
  ...
  testEvm("Should produce expected CREATE2 addresses", async () => {
    ...
    await expect(
      await create2Factory.getFunction("deploy").send(salt),
    ).toBeAccepted();
    ...
    const counterAddress = await create2Factory
    ...
    await expect(
      await counter.getFunction("increment").send(1n),
    ).toBeAccepted();
  ...
  testNoEvm("Should error on EVM contract deployment", async () => {
    ...
    const errorMatcher =
      providerKind === ProviderKind.ZKSYNC ? "toAddressIsNull" : undefined;
...
describe.each([
  ProviderKind.ZKSYNC,
  ProviderKind.ETHERS,
  ProviderKind.L1_ETHERS,
])("EVM contract checks (provider: %s): zkos", describeEvm);
