...
/// [`StorageLog`] together with a corresponding enumeration index of the tree leaf.
// FIXME: simplify (combine with TreeInstruction?)
#[derive(Debug, Clone, PartialEq)]
pub struct StorageLogWithIndex {
    /// Wrapped storage log.
    pub inner: StorageLog,
    /// 1-based enumeration index of the tree leaf.
    pub leaf_index: u64,
}
...
impl ZkSyncTree {
    ...
    pub fn process_genesis_batch(storage_logs: &[StorageLogWithIndex]) -> BlockOutput {
    ...
    pub fn process_l1_batch(&mut self, storage_logs: &[StorageLogWithIndex]) -> TreeMetadata {
        match self.mode {
    ...
    fn process_l1_batch_full(&mut self, storage_logs: &[StorageLogWithIndex]) -> TreeMetadata {
        ...
        for (log, (key, instruction)) in output.logs.iter().zip(&instructions) {
            ...
            let value_written = match instruction {
                TreeInstruction::Write(entry) => entry.value_hash.0,
            ...
            let log = StorageLogMetadata {
                ...
                leaf_enumeration_index: match instruction {
                    ...
                    TreeInstruction::Read => match log.base {
                        TreeLogEntry::Read(prev_entry) => prev_entry.leaf_index,
                ...
                value_read: match log.base {
                    TreeLogEntry::Updated(prev_entry) => {
                        if prev_entry.value_hash.0 == value_written {
                            // A no-op update that must be omitted from the produced `witness`.
                            continue;
                        }
                        prev_entry.value_hash.0
                    }
                    TreeLogEntry::Read(prev_entry) => prev_entry.value_hash.0,
    ...
    fn transform_logs(storage_logs: &[StorageLogWithIndex]) -> Vec<(StorageKey, TreeInstruction)> {
        let instructions = storage_logs.iter().map(|log| {
            let key = log.inner.key;
            let instruction = match log.inner.kind {
                StorageLogKind::Write => {
                    TreeInstruction::Write(TreeEntry::new(log.leaf_index, log.inner.value))
                }
    ...
    fn extract_writes(
        ...
        kvs: impl Iterator<Item = (StorageKey, TreeEntry)>,
    ) -> (
        ...
        for (log_entry, (key, entry)) in logs.zip(kvs) {
            match log_entry {
                TreeLogEntry::Inserted => {
                    ...
                    state_diffs.push(StateDiffRecord {
                        ...
                        final_value: h256_to_u256(entry.value_hash),
                    });
                }
                TreeLogEntry::Updated(prev_entry) => {
                    if prev_entry.value_hash != entry.value_hash {
                        ...
                        state_diffs.push(StateDiffRecord {
                            ...
                            initial_value: h256_to_u256(prev_entry.value_hash),
                            final_value: h256_to_u256(entry.value_hash),
                        });
                    }
                    // Else we have a no-op update that must be omitted from `repeated_writes`.
                }
                TreeLogEntry::Read(_) | TreeLogEntry::ReadMissingKey => {}
            }
        }
    ...
    fn process_l1_batch_lightweight(
        &mut self,
        storage_logs: &[StorageLogWithIndex],
    ) -> TreeMetadata {
        let kvs = Self::filter_write_logs(storage_logs);
        let l1_batch_number = self.next_l1_batch_number();
        tracing::info!(
    ...
    fn filter_write_logs(storage_logs: &[StorageLogWithIndex]) -> Vec<(StorageKey, TreeEntry)> {
        let kvs = storage_logs.iter().filter_map(|log| match log.inner.kind {
            StorageLogKind::Write => {
                let key = log.inner.key;
                Some((key, TreeEntry::new(log.leaf_index, log.inner.value)))
            }
