use std::{
    collections::{hash_map::DefaultHasher, HashSet}, hash::{Hash, Hasher}, sync::Arc, time::Instant
};
...
pub async fn generate_witness(
    ...
    let artifacts_receiver_handle = thread::spawn(move || {
        while let Ok(artifact) = artifacts_receiver.recv() {
            match artifact {
                WitnessGenerationArtifact::BaseLayerCircuit(circuit) => circuit_sender.blocking_send(circuit).unwrap(),
                WitnessGenerationArtifact::RecursionQueue(queue) => queue_sender.blocking_send(queue).unwrap(),
                _ => {}
            }
        }
    });
    ...
    let circuit_receiver_handle = async {
            ...
            .await
        {
            ...
            let partial_circuit_aux_data = None;

            save_circuit_handles.push(tokio::task::spawn_blocking(move || {
                let (circuit_id, circuit_url) = Handle::current().block_on(save_circuit(
                    block_number,
                    circuit,
                    sequence,
