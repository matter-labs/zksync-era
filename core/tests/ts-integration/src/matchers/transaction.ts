...
export async function toBeAccepted(
  ...
  try {
    ...
    reporter?.debug(
      `Waiting for transaction ${tx.hash} (from=${tx.from}, nonce=${tx.nonce}) to get accepted`,
    );
...
export async function toBeReverted(
  txPromise:
    | AugmentedTransactionResponse
    | Promise<AugmentedTransactionResponse>,
  modifiers: MatcherModifier[] = [],
  additionalInfo?: string,
) {
  try {
    ...
    const reporter = tx.reporter;
    reporter?.debug(
      `Waiting for transaction ${tx.hash} (from=${tx.from}, nonce=${tx.nonce}) to get reverted`,
    );
...
async function toBeRejectedWithPrefix(
  ...
  try {
    ...
    const message = new TestMessage()
      ...
      .line(
        "Transaction was expected to be rejected by the API server, but it was not. Receipt:",
      )
      .received(tx)
      .additional(additionalInfo)
      .build();
    ...
    if (errorSubstring) {
      ...
      if (!error.message || !error.message.includes(fullErrorSubstring)) {
        const message = new TestMessage()
          .matcherHint(".toBeRejected")
          .line(
            "Transaction was expected to be rejected by the API server with the following message:",
          )
    ...
    if (dataSubstring) {
      ...
      if (!error.message || !error.message.includes(dataSubstring)) {
        const message = new TestMessage()
          .matcherHint(".toBeRejected")
          .line(
            "Transaction was expected to be rejected by the API server with the following data:",
          )
 ...
 */
function checkReceiptFields(
  request: ethers.TransactionResponseParams,
  ...
  const failWith = (line: string) =>
    fail(errorMessageBuilder.line(line).build());

  if (receipt.status !== 0 && receipt.status !== 1) {
    return failWith(
      `Status field in the receipt has an unexpected value (expected 0 or 1): ${receipt.status}`,
    );
  }
