...
use crate::{
    apply_l1_to_l2_alias, consts::INITIAL_DEPLOYMENT_FILE, forge_interface::deploy_ecosystem::input::InitialDeploymentConfig, traits::{FileConfigWithDefaultName, ZkToolboxConfig}, ChainConfig, ContractsConfig, GenesisConfig, WalletsConfig, ERC20_DEPLOYMENT_FILE
};
...
impl GatewayChainUpgradeInput {
    pub fn new(
        ...
        let validum = current_chain_config.get_genesis_config().unwrap().l1_batch_commit_data_generator_mode == L1BatchCommitmentMode::Validium;


        Self {
            owner_address: current_chain_config.get_wallets_config().unwrap().governor.address,
