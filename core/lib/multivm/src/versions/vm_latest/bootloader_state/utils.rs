...
pub(super) fn apply_tx_to_memory(
    ...
    let bootloader_description_offset =
        get_bootloader_tx_description_offset(subversion) + BOOTLOADER_TX_DESCRIPTION_SIZE * tx_index;
    ...
    apply_l2_block_inner(memory, bootloader_l2_block, tx_index, start_new_l2_block, subversion);

    // Note, +1 is moving for pointer
    let compressed_bytecodes_offset = get_compressed_bytecodes_offset(subversion) + 1 + compressed_bytecodes_size;
...
fn apply_l2_block_inner(
    ...
    let block_position =
        get_tx_operator_l2_block_info_offset(subversion) + txs_index * TX_OPERATOR_SLOTS_PER_L2_BLOCK_INFO;
...
pub(crate) fn apply_pubdata_to_memory(
    ...
    let (l1_messenger_pubdata_start_slot, pubdata) = match subversion {
        MultiVMSubversion::SmallBootloaderMemory | MultiVMSubversion::IncreasedBootloaderMemory => {
            ...
            let l1_messenger_pubdata_start_slot = get_operator_provided_l1_messenger_pubdata_offset(subversion) + 2;
        ...
        MultiVMSubversion::Gateway => {
            ...
            let l1_messenger_pubdata_start_slot = get_operator_provided_l1_messenger_pubdata_offset(subversion) + 1;
