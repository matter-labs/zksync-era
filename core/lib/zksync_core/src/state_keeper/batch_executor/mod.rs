...
use multivm::{HistoryEnabled, MultivmTracer, VmInstance, VmInstanceData};
use vm::{
    CallTracer, ExecutionResult, FinishedL1Batch, Halt, L1BatchEnv, L2BlockEnv, Snapshot,
...
pub(super) struct BatchExecutor {
    ...
    snapshot_before_last_tx: Option<Snapshot>,
}
...
impl BatchExecutor {
    ...
    fn execute_tx<S: ReadStorage>(
        ...
        // Save VM snapshot in case the revert transaction command is used
        // Forget the previous snapshot to save memory
        if let Some(snapshot) = self.snapshot_before_last_tx.take() {
            vm.pop_snapshot_no_rollback(snapshot)
        }
        self.snapshot_before_last_tx = Some(vm.make_snapshot());
    ...
    fn rollback_last_tx<S: ReadStorage>(&mut self, vm: &mut VmInstance<'_, S, HistoryEnabled>) {
        let stage_started_at = Instant::now();
        vm.rollback_to_the_latest_snapshot(self.snapshot_before_last_tx.take().unwrap());
        metrics::histogram!(
            "server.state_keeper.tx_execution_time",
            stage_started_at.elapsed(),
            "stage" => "tx_rollback"
