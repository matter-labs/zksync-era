...
function isIgnored(err: any): boolean {
  ...
  return IGNORED_ERRORS.some(
    (sampleErr) => errString.indexOf(sampleErr) !== -1,
  );
}
...
export function createFetchRequest(
  ...
  fetchRequest.getUrlFunc = async (
    req: ethers.FetchRequest,
    signal?: ethers.FetchCancelSignal,
  ) => {
    ...
    for (let retry = 0; retry < 50; retry++) {
      try {
        ...
        if (retry != 0) {
          reporter?.debug(
            `RPC request ${req} took ${retry} retries to succeed`,
          );
        }
        return result;
      } catch (err: any) {
    ...
    return Promise.reject(
      new Error(`Retried too many times, giving up on request=${req}`),
    );
  };
  return fetchRequest;
}
...
export class RetryProvider extends zksync.Provider {
  ...
  override async send(method: string, params: any): Promise<any> {
    for (let retry = 0; retry < 50; retry++) {
      try {
        ...
        if (retry != 0) {
          this.reporter?.debug(
            `Request for method ${method} took ${retry} retries to succeed`,
          );
        }
        return result;
      } catch (err: any) {
  ...
  override _wrapTransactionReceipt(
    receipt: any,
  ): zksync.types.TransactionReceipt {
...
class L2TransactionResponse
  ...
  implements AugmentedTransactionResponse
{
...
class EthersTransactionResponse
  extends ethers.TransactionResponse
  implements AugmentedTransactionResponse
{
  private isWaitingReported: boolean = false;
  private isReceiptReported: boolean = false;
  ...
  override replaceableTransaction(
    startBlock: number,
  ): EthersTransactionResponse {
    const base = super.replaceableTransaction(startBlock);
...
export class RetryableWallet extends zksync.Wallet {
  constructor(
    privateKey: string,
    l2Provider: RetryProvider,
    l1Provider: EthersRetryProvider,
  ) {
    super(privateKey, l2Provider, l1Provider);
  }

  override ethWallet(): RetryableL1Wallet {
    return new RetryableL1Wallet(
      this.privateKey,
      <EthersRetryProvider>this._providerL1(),
    );
  }
...
function debugData(rawData: string): string {
  ...
  return byteLength <= MAX_PRINTED_BYTE_LEN
    ? rawData
    : rawData.substring(0, 2 + MAX_PRINTED_BYTE_LEN * 2) +
        `...(${byteLength} bytes)`;
}
