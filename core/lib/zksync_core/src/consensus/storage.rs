...
impl<'a> StorageProcessor<'a> {
    ...
    async fn fetch_sync_block(
        ...
        Ok(ctx
            .wait(self.0.sync_dal().sync_block(number, true))
            .await?
            .context("sync_block()")?)
    }
    ...
    pub async fn fetch_block(
        ...
        let Some(block) = self.fetch_sync_block(ctx, number).await? else {
            return Ok(None);
        };
    ...
    pub async fn fetch_payload(
        ...
        let Some(sync_block) = self.fetch_sync_block(ctx, block_number).await? else {
            return Ok(None);
        };
    ...
    pub async fn put_block(
        ...
        let sync_block = txn
            .fetch_sync_block(ctx, block.header.number)
            .await?
