...
mod tests {
    ...
    fn splitting_patch_set() {
        ...
        let all_nibbles = (1_u8..=255).map(|i| {
            ...
            let leaf = LeafNode::new(key, TreeEntry::new(i.into(), ValueHash::zero()));
    ...
    fn loading_greatest_key() {
        ...
        let (_, patch) = Storage::new(&db, &(), 0, true)
            .extend(vec![(key, TreeEntry::new(1, ValueHash::zero()))]);
        ...
        let (_, patch) = Storage::new(&db, &(), 1, true)
            .extend(vec![(other_key, TreeEntry::new(2, ValueHash::zero()))]);
        db.apply_patch(patch);
        ...
        let (_, patch) = Storage::new(&db, &(), 2, true)
            .extend(vec![(greater_key, TreeEntry::new(3, ValueHash::zero()))]);
        db.apply_patch(patch);
