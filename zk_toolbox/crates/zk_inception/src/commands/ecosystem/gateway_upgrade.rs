...
use config::{
    forge_interface::{
        deploy_ecosystem::{
            ...
            output::ERC20Tokens,
        }, gateway_ecosystem_upgrade::{input::GatewayEcosystemUpgradeInput, output::GatewayEcosystemUpgradeOutput}, script_params::{DEPLOY_ERC20_SCRIPT_PARAMS, GATEWAY_UPGRADE_ECOSYSTEM_PARAMS}
    }, traits::{FileConfigWithDefaultName, ReadConfig, ReadConfigWithBasePath, SaveConfig, SaveConfigWithBasePath}, ContractsConfig, EcosystemConfig, GenesisConfig, WalletsConfig, CONFIGS_PATH
};
...
use super::{
    args::{gateway_upgrade::{GatewayUpgradeArgs, GatewayUpgradeArgsFinal}, init::{EcosystemArgsFinal, EcosystemInitArgs, EcosystemInitArgsFinal}},
    common::deploy_l1,
    setup_observability,
...
use crate::{
    ...
    commands::{
        chain::{self},
        ecosystem::{args::gateway_upgrade::GatewayUpgradeStage, create_configs::{
...
async fn no_governance_prepare(
    ...
    let ecosystem_upgrade_config_path = GATEWAY_UPGRADE_ECOSYSTEM_PARAMS.input(&ecosystem_config.link_to_code);
    ...
    let mut forge = Forge::new(&ecosystem_config.path_to_l1_foundry())
        .script(&GATEWAY_UPGRADE_ECOSYSTEM_PARAMS.script(), forge_args.clone())
        .with_ffi()
