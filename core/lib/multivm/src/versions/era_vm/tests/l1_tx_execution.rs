...
use crate::{
    era_vm::{
        tests::{
            ...
            utils::{read_test_contract, zk_storage_key_to_lambda, BASE_SYSTEM_CONTRACTS},
        },
...
fn test_l1_tx_execution() {
    ...
    for (expected_value, storage_location) in [
        ...
        assert_eq!(expected_value, *actual_value.unwrap_or(&U256::zero()));
    }
    ...
    let res = vm.vm.execute(VmExecutionMode::OneTx);
    let storage_logs: Vec<zksync_types::StorageLogWithPreviousValue> = res.logs.storage_logs;
    let res = StorageWritesDeduplicator::apply_on_empty_state(&storage_logs);
