...
impl ProtoRepr for proto::WitnessGenerator {
    ...
    fn read(&self) -> anyhow::Result<Self::Type> {
        Ok(Self::Type {
            ...
            initial_setup_key_path: required(&self.initial_setup_key_path).context("initial_setup_key_path")?.clone(),
            key_download_url: required(&self.key_download_url).context("key_download_url")?.clone(),
            max_attempts: *required(&self.max_attempts).context("max_attempts")?,
            blocks_proving_percentage: self.blocks_proving_percentage.map(|x|x.try_into()).transpose().context("blocks_proving_percentage")?,
            dump_arguments_for_blocks: self.dump_arguments_for_blocks.clone(),
            last_l1_batch_to_process: self.last_l1_batch_to_process,
            data_source: required(&self.data_source).and_then(|x|Ok(proto::BasicWitnessGeneratorDataSource::try_from(*x)?)).context("data_source")?.parse(),
        })
    }
    ...
    fn build(this: &Self::Type) -> Self {
        Self {
            ...
            data_source: Some(proto::BasicWitnessGeneratorDataSource::new(&this.data_source).into()),
        }
    }
}
