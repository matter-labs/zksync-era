...
impl JobProcessor for CircuitSynthesizer {
    ...
    async fn get_next_job(
        ...
        let prover_job = connection_pool
    ...
    async fn save_result(
        ...
        vlog::info!("Finished circuit synthesis for job: {}", job_id);
        let config: CircuitSynthesizerConfig = CircuitSynthesizerConfig::from_env();
        let (assembly, circuit_id) = artifacts;
        let now = Instant::now();
        let specialized_prover_group_id = ProverGroupConfig::from_env()
            .get_group_id_for_circuit_id(circuit_id)
            .unwrap_or_else(|| panic!("No specialized prover running for circuit: {}", circuit_id));
        while now.elapsed() < config.prover_instance_wait_timeout() {
            let optional_prover_instance = pool
                .clone()
                .access_storage_blocking()
                .gpu_prover_queue_dal()
                .get_free_prover_instance(config.gpu_prover_queue_timeout(), specialized_prover_group_id);
