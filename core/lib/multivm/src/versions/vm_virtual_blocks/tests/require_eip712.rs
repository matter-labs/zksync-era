...
async fn test_require_eip712() {
    ...
    let (tx_request, hash) = TransactionRequest::from_bytes(&aa_tx, 270.into()).unwrap();
    ...
    let domain = Eip712Domain::new(chain_id.into());
    let signature = private_account
        .get_pk_signer()
        .sign_typed_data(&domain, &transaction_request)
        .await
        .unwrap();
    let encoded_tx = transaction_request.get_signed_bytes(&signature, chain_id.into());
