...
/// Obtains the test master database URL from the environment variable.
fn get_test_master_database_url() -> anyhow::Result<String> {
...
/// Obtains the test prover database URL from the environment variable.
fn get_test_prover_database_url() -> anyhow::Result<String> {
...
pub async fn create_test_db() -> anyhow::Result<url::Url> {
    use rand::Rng as _;
    use sqlx::{Connection as _, Executor as _};
    const PREFIX: &str = "test-";
    let db_url = get_test_master_database_url().unwrap();
...
pub async fn create_test_prover_db() -> anyhow::Result<url::Url> {
    use rand::Rng as _;
    use sqlx::{Connection as _, Executor as _};
    const PREFIX: &str = "test-";
...
#[derive(Clone)]
pub struct ConnectionPool(pub PgPool);
...
impl ConnectionPool {
    pub async fn test_pool<S: StorageProcessor>() -> ConnectionPool {
        let type_name = std::any::type_name::<S>();

        let db_url: String;
        if type_name.contains("ProverStorageProcessor") {
            db_url = create_test_prover_db()
                .await
                .expect("Unable to prepare test database")
                .to_string();
        } else if type_name.contains("ServerStorageProcessor") {
            db_url = create_test_db()
                .await
                .expect("Unable to prepare test database")
                .to_string();
        } else {
    ...
    pub async fn access_storage<S: StorageProcessor>(&self) -> anyhow::Result<S> {
        self.access_storage_inner::<S>(None).await
    }
    ...
    pub async fn access_storage_tagged<S: StorageProcessor>(
        &self,
        requester: &'static str,
    ) -> anyhow::Result<S> {
        self.access_storage_inner::<S>(Some(requester)).await
    }

    async fn access_storage_inner<S: StorageProcessor>(
        &self,
